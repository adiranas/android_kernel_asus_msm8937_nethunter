...
static struct regulator *vdd_switch = NULL;
static bool vdd_switch_enable = false;
...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//printk("zjd,spidev_ioctl!\n");
	/* Check type and command number */
	if (_IOC_TYPE(cmd) != SPI_IOC_MAGIC)
	{
		//printk("zjd,_IOC_TYPE(cmd) != SPI_IOC_MAGIC!\n");
		return -ENOTTY;
	}
	...
	{
		//printk("zjd,_IOC_DIR(cmd) & _IOC_READ!\n");
		err = !access_ok(VERIFY_WRITE,
				(void __user *)arg, _IOC_SIZE(cmd));
	}
	if (err == 0 && _IOC_DIR(cmd) & _IOC_WRITE)
	{
		//printk("zjd,_IOC_DIR(cmd) & _IOC_WRITE\n");
	...
	switch (cmd) {
	case SPI_IOC_WR_DTV_FM_SWITCH:
		retval = __get_user(tmp, (u8 __user *)arg);
		if (retval == 0) {
			if(tmp == SW_TO_DTV_WW) {
				if(regulator_is_enabled(vdd_switch) <= 0) {
					rc = regulator_enable(vdd_switch);
					if(rc) {
						//printk("zjd, error on enable vreg_switch");
					} else {
						//printk("zjd, vreg_switch enable success!");
						vdd_switch_enable = true;
					}
				} else {
					//printk("zjd, vreg_switch already enable before");
				}

				rc = gpio_request(98,"DTV_FM_RF_SW");
				if (!rc) {
				...
				rc = gpio_request(61,"DTV_UHF_VHF_SW_2");
				if (!rc) {
				...
				rc = gpio_request(63,"DTV_UHF_VHF_SW_1");
				if (!rc) {
	...
	case SPI_IOC_WR_TUNER_POWER:
		//printk("zjd,SPI_IOC_WR_TUNER_POWER!\n");
		retval = __get_user(tmp, (u8 __user *)arg);
		//printk("zjd,SPI_IOC_WR_TUNER_POWER retval == %d tmp == %d\n",retval, tmp);
		if (retval == 0) {
			if(tmp == SPI_TUNER_POWER_ON) {
				rc = gpio_request(50,"DTV_1V8_EN");
				if (!rc) {
				...
				gpio_free(33);
				//printk("zjd,SPI_IOC_WR_TUNER_POWER power on ok!\n");
			} else {
				rc = gpio_request(50,"DTV_1V8_EN");
				if (!rc) {
				...
				gpio_free(33);
				//printk("zjd,SPI_IOC_WR_TUNER_POWER power off ok!\n");
			}
		}
		break;
	/* read requests */
	...
	case SPI_IOC_WR_MODE32:
		...
		if (retval == 0) {
			...
			if (retval < 0){
				spi->mode = save;
				//dev_info(&spi->dev, "zjd,set spi mode fail%x\n", save);
			}
			else
...
static int spidev_probe(struct spi_device *spi)
{
	...
	//printk("zjd, spidev_probe\n");
	/* Allocate driver data */
	spidev = kzalloc(sizeof(*spidev), GFP_KERNEL);
	if (!spidev)
		return -ENOMEM;

	vdd_switch = regulator_get(&spi->dev,"vdd_switch");
	if (!vdd_switch) {
		printk("Unable to get vdd_switch\n");
	}
	...
	//printk("zjd, spidev_probe out,status == %d\n",status);
	return status;
}
...
static int spidev_remove(struct spi_device *spi)
{
	...
	if(vdd_switch_enable) {
		regulator_disable(vdd_switch);
		vdd_switch_enable = false;
	}
	regulator_put(vdd_switch);
	vdd_switch = NULL;
...
{
	int status;
	//printk("zjd, tan test spidev_init===============\n");
	...
	//printk("zjd, tan test spidev_init=======ok========\n");
	return 0;
error_mem:
error_busnum:
