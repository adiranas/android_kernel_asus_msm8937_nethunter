		...
		const char *psy_name,  const char  *batt_type)
{
	...
	bool in_range = false;
	char batt_name[30] = "itech_3000mah_4200mv";
	...
	printk("battery range is %dkohm-%dkohm\n",batt_id_kohm*(100-id_range_pct)/100,batt_id_kohm*(100-id_range_pct)/100);
	if(g_ASUS_PRJ_ID == PRJ_SCORPIO_5W || g_ASUS_PRJ_ID == PRJ_SCORPIO_10W){
		memset(batt_name, 0, sizeof(batt_name));
		if(batt_id_kohm >= 8 && batt_id_kohm <= 12)
		{
			strcpy(batt_name,"scorpio_2nd_source");
		}else{
			strcpy(batt_name,"scorpio_main_source");
		}
	}
