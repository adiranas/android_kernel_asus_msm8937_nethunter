...
struct qpnp_hap {
	...
	int asus_index; //bsp +++ guochang_qiu
};
...
//asus bsp guochang_qiu++++
static int vmax_value[7] = {116, 300, 600, 900, 1000, 1100, 1100};
static ssize_t qpnp_hap_asus_vmax_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	if (sscanf(buf, "%d", &index) != 1){
		pr_err("get vmax from user failed!\n");
		return -EINVAL;
	}

	if(index < 0)
		index = 0;
	else if(index >= ARRAY_SIZE(vmax_value))
		index = ARRAY_SIZE(vmax_value) - 1;

	if(hap->asus_index == index)
		goto DONE;

	pr_info("vibrator index=%d\n", index);
	hap->asus_index = index;
	hap->vmax_mv = vmax_value[index];
...
static ssize_t qpnp_hap_asus_vmax_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_hap *hap = container_of(timed_dev, struct qpnp_hap,
	...
	for(index=0; index< ARRAY_SIZE(vmax_value); index++)
	{
		if(vmax_value[index] == hap->vmax_mv)
			break;
	}
...
static struct device_attribute qpnp_hap_attrs[] = {
	...
	//asus bsp guochang_qiu++++
	__ATTR(asus_vmax, (S_IRUGO | S_IWUSR | S_IWGRP),
			qpnp_hap_asus_vmax_show,
			qpnp_hap_asus_vmax_store),
	//asus bsp guochang_qiu---
};
...
static int qpnp_haptic_probe(struct spmi_device *spmi)
{
	...
	//bsp +++ guochang_qiu adjust the vibrator intensity
	hap->asus_index = -1;
	//bsp +++ guochang_qiu adjust the vibrator intensity
