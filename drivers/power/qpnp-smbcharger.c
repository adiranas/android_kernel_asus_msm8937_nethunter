...
bool smb_chg_is_charging(int usb_state);
int asus_battery_update_status(void);
//asus report capacity frank ----

static struct alarm jeita_alarm;
struct delayed_work SetJeitaRTCWorker;
extern unsigned long get_chg_temp(void);

int thermal_test_enable = 0;
int soft_jeita_disable;
bool disable_jeita_alarm;

struct smbchg_chip *smb_charger_dev;

struct timespec thermal_engine_control_time;
...
#define asus_adaptor_detect_soc 3
...
static char *asus_charging_type_str[]={
	...
	[DCP]	="DCP",
	[CDP]	="CDP",
	[FLOATING]	="FlOATING_0P5A",
	[UNDEFINED]	="UNDEFINED",
	[PB_2A]	="DCP_PB_2A",
	[ASUS_750K]	="DCP_ASUS_750K_2A",
	[ASUS_200K]	="DCP_ASUS_200K_2A",
	[RESERVE_390K]	="DCP_OTHERS_1A",
	[RESERVE_100K]	="DCP_OTHERS_1A",
	[DCP_OTHERS]	="DCP_OTHERS_1A",
	[OTHERS]	="OTHERS_1A",
	[NONE]	="NONE",
};
...
struct smbchg_chip {
	...
	//guochang_qiu ATD_test++++
#ifdef ASUS_FACTORY_BUILD
	struct delayed_work		asus_charging_limit_work;
#endif
	//guochang_qiu ATD_test----
	bool					force_rerun_apsd;
	enum JEITA_state_for_all soft_jeita_state;
	enum asus_charging_type asus_charging_type;
	int cancel_stop_charging;
	struct delayed_work	demo_app_work;
...
#define disable_charging_PROC_FILE "driver/disable_charging"
#define enable_charging_PROC_FILE "driver/enable_charging"
static int charging_limit_cap = 65;
static bool disable_charging = false;
static void charging_limit_check(struct work_struct *works);
bool factory_limit_disable_charging;
#endif
//guochang_qiu ATD_test----
...
struct wake_lock asus_soc_check_wake_lock;
struct wake_lock asus_jeita_alarm_wake_lock;
...
extern void goodix_usb_detection(bool plugin);         //ASUS BSP Ander: notify touch cable in +++
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_info( "Couldn't get capacity rc = %d\n", rc);
...
static int get_prop_batt_current_now(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_info("Couldn't get current rc = %d\n", rc);
		ua = DEFAULT_BATT_CURRENT_NOW;
	}

	//add bsp guochang_qiu +++
	rc = smbchg_read(chip, &reg, chip->chgr_base + CHGR_STS, 1);
	if (rc < 0) {
		dev_err(chip->dev, "Unable to read CHGR_BASE rc = %d\n", rc);
	}

	charger_present = is_usb_present(chip);

	if(charger_present && reg&0x20 && (ua <= 5*1000 && ua >= -5*1000) && (get_prop_batt_capacity(chip) == 100))
		ua = DEFAULT_BATT_CURRENT_NOW;
	//add bsp guochang_qiu +++
...
{
	if(g_charging_toggle_status != en)
	{
		g_charging_toggle_status = en;
		pr_info("g_charging_toggle_status changed to %d;\n", en);
	}
...
//	pr_smb(PR_STATUS, "USB current_ma = %d\n", current_ma);
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (current_ma == CURRENT_500_MA) {
			if(chip->asus_charging_type == FLOATING)
			{
				pr_info("floating charging setting \n");
				// 1. 0x13F2 = 0x04
				rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + 0xF2,
						SMB_MASK(7,0),BIT(2));
				if (rc < 0) {
					dev_err(chip->dev, "Couldn't set 0x13f2 rc=%d\n", rc);
				}

				// 2. 0x1340 = 0x05
				rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + 0x40,
						SMB_MASK(7,0),BIT(2)|BIT(0));
				if (rc < 0) {
					dev_err(chip->dev, "Couldn't set 0x1340 rc=%d\n", rc);
				}
...
out:
	pr_info("usb type = %d current set to %d mA\n",
							...
							int current_ma)
{
	...
	if (chip->tables.usb_ilim_ma_table[i] == chip->fastchg_current_ma) {
		pr_info("skipping fastchg current request: %d\n",
...
//	int thermal_icl_ma;
	...
	thermal_engine_control_time = current_kernel_time();
	...
	if(chip->cancel_stop_charging)
		goto out;

#ifdef ASUS_FACTORY_BUILD
	pr_info("SMBCHG:this is factory image!\n");
	goto out;//factory image stop charger policy
#endif


	prev_therm_lvl = chip->therm_lvl_sel;
	chip->therm_lvl_sel = lvl_sel;
	if (chip->therm_lvl_sel == (chip->thermal_levels - 1)) { //thermal level = 3
		/*
	...
	else if (chip->therm_lvl_sel == 0 || chip->therm_lvl_sel == 1) //thermal level = 0 or 1
	{
	...
	else if (chip->therm_lvl_sel == 2) //thermal level = 2
	{
		rc = vote(chip->usb_suspend_votable, THERMAL_EN_VOTER,
								false, 0);
		if (rc < 0) {
		...
		enum led_brightness value)
{
	...
	if(chip->led_reg){
		if((value > LED_OFF)&&(!chip->led_reg_enable)){
			rc = regulator_set_voltage(chip->led_reg,
						chip->led_reg->min_uV, chip->led_reg->max_uV);
			if (rc) {
				dev_err(&chip->spmi->dev,
					"CHG_LED regulator voltage set failed(%d)\n",rc);
				goto err_reg_vtg_set;
			}

			rc = regulator_enable(chip->led_reg);
			if (rc) {
				dev_err(&chip->spmi->dev,"Regulator enable failed(%d)\n", rc);
				goto err_reg_enable;
			}

			chip->led_reg_enable=1;
		}else if((value <= LED_OFF)&&(chip->led_reg_enable)){
			rc = regulator_disable(chip->led_reg);
			if (rc) {
				dev_err(&chip->spmi->dev,"Regulator disable failed(%d)\n", rc);
				return;
			}

			rc = regulator_set_voltage(chip->led_reg,
						0, chip->led_reg->max_uV);
			if (rc) {
				dev_err(&chip->spmi->dev,
					"CHG_LED regulator voltage set failed(%d)\n",
					rc);
			...
			chip->led_reg_enable=0;
		}
	}
	return;
err_reg_enable:
	if(chip->led_reg)
		regulator_set_voltage(chip->led_reg,0, chip->led_reg->max_uV);
	chip->led_reg_enable=0;
err_reg_vtg_set:
	pr_smb(PR_STATUS, "error set chg_led regulator\n");
}
		...
		unsigned long blinking)
{
	...
	if (rc)
		dev_err(chip->dev, "Couldn't write CHG_LED rc=%d\n",
				rc);
	if(chip->led_reg){
		if(blinking && (!chip->led_reg_enable)){
			rc = regulator_set_voltage(chip->led_reg,
						chip->led_reg->min_uV, chip->led_reg->max_uV);
			if (rc) {
				dev_err(&chip->spmi->dev,
			...
			chip->led_reg_enable=1;
		}else if((!blinking) && (chip->led_reg_enable)){
			rc = regulator_disable(chip->led_reg);
			if (rc) {
				dev_err(&chip->spmi->dev,"Regulator disable failed(%d)\n", rc);
				return;
						...
						enum power_supply_type type)
{
	...
	if (chip->typec_psy && (type != POWER_SUPPLY_TYPE_USB))
		...
		{
			if(g_ASUS_PRJ_ID == PRJ_CAPRICORN_5W||
			g_ASUS_PRJ_ID == PRJ_SCORPIO_5W||
			g_ASUS_PRJ_ID == PRJ_GEMINI_5W||
			(g_ASUS_PRJ_ID == PRJ_SCORPIO_10W && g_ASUS_PRJ_STAGE >= STAGE_ER))
			{
		...
		{
			if(g_ASUS_PRJ_ID == PRJ_CAPRICORN_10W||
			(g_ASUS_PRJ_ID == PRJ_SCORPIO_10W && g_ASUS_PRJ_STAGE < STAGE_ER)||
			g_ASUS_PRJ_ID == PRJ_GEMINI_10W)
			{
				pr_info("10W project,set 2A input current\n");
				current_limit_ma = CURRENT_LIMIT_2000MA;
			}
			else
				if(g_ASUS_PRJ_ID == PRJ_CAPRICORN_5W||
				g_ASUS_PRJ_ID == PRJ_SCORPIO_5W||
				...
				(g_ASUS_PRJ_ID == PRJ_SCORPIO_10W && g_ASUS_PRJ_STAGE >= STAGE_ER))
			{
				pr_info("5W project,set 1A input current\n");
				current_limit_ma = CURRENT_LIMIT_1000MA;
			}
			else
			{
				pr_info("unknow project,set 1A input current\n");
				current_limit_ma = CURRENT_LIMIT_1000MA;
			}
		}
	}
	...
	pr_info("Type %d: asus_charging_type => %s setting mA = %d\n",
...
//add uts USB plug-in/out log guochang_qiu +++
static int enable_uts_log = 1;
static void UTS_chg_mode(enum power_supply_type usb_supply_type, struct smbchg_chip *chip)
{
	int local_type = -1;
	char cable_chg_mode[9][8] = {
		"USB",
		"CDP",
		"ASUS AC",
		"None",
	...
	ASUSEvtlog("[USB] set_chg_mode: %s\n", cable_chg_mode[local_type]);
}
//add uts USB plug-in/out log guochang_qiu ---


#define HVDCP_NOTIFY_MS		2500
#define ASUS_DETECT_MS		30000
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	//add uts USB plug-in/out log guochang_qiu +++
	if(enable_uts_log == 1)
		UTS_chg_mode(usb_supply_type, chip);
	//add uts USB plug-in/out log guochang_qiu ---
		...
		if(thermal_test_enable == 0)
		{
			...
			g_ASUS_PRJ_ID == PRJ_GEMINI_10W)
			{
				...
				if(capacity <= asus_adaptor_detect_soc)
				{
					...
					wake_lock(&asus_soc_check_wake_lock);
				}
			}
			else
			{
				chip->asus_charging_type = DCP_OTHERS;
			}
		}
	}
	...
	if (!chip->usb_ov_det) {
		 ...
		 */
		pr_info("setting usb psy health %s\n",
	...
	if(usb_supply_type == POWER_SUPPLY_TYPE_USB)
	{
		...
		schedule_delayed_work(&chip->asus_sdp_det_work,2*HZ);
		chip->asus_charging_type = SDP;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if(g_ASUS_PRJ_ID == PRJ_SCORPIO_10W || g_ASUS_PRJ_ID == PRJ_SCORPIO_5W)
		{
			val->intval = 3000;
		}
		else if(g_ASUS_PRJ_ID == PRJ_GEMINI_10W || g_ASUS_PRJ_ID == PRJ_GEMINI_5W)
		{
			val->intval = 2400;
		}
		else
...
static irqreturn_t vbat_low_handler(int irq, void *_chip)
{
	...
	//bsp guochang_qiu add ASUS Event log +++
...
static irqreturn_t usbin_ov_handler(int irq, void *_chip)
{
	...
	if (reg & USBIN_OV_BIT) {
		...
		if (chip->usb_psy) {
			pr_info("setting usb psy health OV\n");
			rc = power_supply_set_health_state(chip->usb_psy,
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	pr_info_ratelimited("%s chip->usb_present = %d rt_sts = 0x%02x hvdcp_3_det_ignore_uv = %d aicl = %d\n",
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT)) {
		pr_warn_ratelimited("Very weak charger detected\n");
		chip->very_weak_charger = true;
		rc = smbchg_read(chip, &reg,
				chip->usb_chgpth_base + ICL_STS_2_REG, 1);
		if (rc) {
...
static int smbchg_check_chg_version(struct smbchg_chip *chip)
{
	...
	case PMI8950:
		...
		//asus guochang_qiu scorpio can not support hvdcp++++
	...
	pr_info("pmic=%s, wa_flags=0x%x, hvdcp_supported=%s\n",
...
extern int get_charger_type(void)
{
	...
	if (!smb_charger_dev) {
		pr_err("Warning: smb_charger_dev is null due to probe function has error\n");
		return -1;
	}
	read_usb_type(smb_charger_dev, &usb_type_name, &usb_supply_type);
	//pr_info("usb_supply_type ==> %d \n",usb_supply_type);
	batt_info.usb_supply_type = usb_supply_type;
	if(smb_charger_dev->usb_present)
	{
		if(usb_supply_type == POWER_SUPPLY_TYPE_USB )
			return 0;
		else if(usb_supply_type == POWER_SUPPLY_TYPE_USB_CDP)
...
int asus_battery_update_status(void)
{
	...
	if (smb_charger_dev->usb_present == 1 && get_prop_batt_capacity(smb_charger_dev) ==100)
	{
		status = POWER_SUPPLY_STATUS_FULL;
	}
	else if (smb_charger_dev->usb_present == 0 && get_prop_batt_capacity(smb_charger_dev) ==100)
	{
		status = POWER_SUPPLY_STATUS_DISCHARGING;
	}
	else
		status = get_prop_batt_status(smb_charger_dev);

	return status;
}

//add uts charger fault log guochang_qiu +++
static void asus_print_charger_fault(u32 cable_status, int charger_status)
{
	if(cable_status != 3 && charger_status != POWER_SUPPLY_STATUS_CHARGING)
	{
		if(charger_status == POWER_SUPPLY_STATUS_UNKNOWN)
			ASUSEvtlog("[BAT] Charger Fault: can not read register\n");
		else if(charger_status == POWER_SUPPLY_STATUS_NOT_CHARGING)
			ASUSEvtlog("[BAT] Charger Fault: hold-off\n");
		else if(charger_status == POWER_SUPPLY_STATUS_DISCHARGING)
...
static int asus_update_all(void)
{
	...
	batt_info.rawsoc = get_prop_batt_capacity(smb_charger_dev);
	batt_info.cable_status = get_charger_type();
	batt_info.charging_toggle = get_sw_charging_toggle();
	batt_info.Rsoc =get_prop_batt_capacity(smb_charger_dev);
...
static int asus_print_all(void)
{
	...
	*/
	...
	snprintf(battInfo, sizeof(battInfo), "%sTemp:%s%d.%dC, Cable:%d(%s), Otg_toggle:%d, Status:%s, Charging:%s, Rsoc:%d, BATID:%lld, TLevel:%d, usb_supply_type:%d\n",
		battInfo,
		batt_info.temperature_negative_sign,
		...
		batt_info.otg_toggle,
		batt_status_str[batt_info.status],
		batt_info.charging_toggle ? "ON" : "OFF",
		batt_info.Rsoc,
		battID,
		smb_charger_dev->therm_lvl_sel,
		batt_info.usb_supply_type);
	ASUSEvtlog("[BAT][Ser]%s", battInfo);

	//add uts charger fault log guochang_qiu +++
	asus_print_charger_fault(batt_info.cable_status, batt_info.status);
	//add uts charger fault log guochang_qiu ---

	BAT_DBG("%s", battInfo);
	g_last_print_time = current_kernel_time();

	if (smb_charger_dev->psy_registered)
		power_supply_changed(&smb_charger_dev->batt_psy);
...
//asus bsp guochang_qiu demp app stop charger++++
...
#define DEMO_APP_STOP_CHARGER 60
#define DEMO_APP_RECHG 55
static void dmeo_app_control_charger(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				demo_app_work.work);
	int capacity_now = get_prop_batt_capacity(chip);

	demo_app_status_flag = ADF_check_status();
	if(demo_app_status_flag && capacity_now >= DEMO_APP_STOP_CHARGER)
		vote(chip->battchg_suspend_votable, DEMO_APP_VOTER, true, 0);//stop charger
	else if(demo_app_status_flag && capacity_now < DEMO_APP_RECHG)
		vote(chip->battchg_suspend_votable, DEMO_APP_VOTER, false, 0);//enable charger
	schedule_delayed_work(&chip->demo_app_work,30*HZ);

	return ;
}

static bool demo_app_charging = false;
static ssize_t demo_app_charger_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	int temp = -1;
	...
	if(temp == 0){
		cancel_delayed_work(&smb_charger_dev->demo_app_work);
		pr_info("exit demo app control charger");
	}else{
		pr_info("enter demo app control charger");
		cancel_delayed_work(&smb_charger_dev->demo_app_work);
		schedule_delayed_work(&smb_charger_dev->demo_app_work, 0);
	}
	demo_app_charging = !!temp;

	return len;
}

static ssize_t demo_app_charger_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", demo_app_charging);
}

static DEVICE_ATTR(demo_app_charger, 0664, demo_app_charger_show, demo_app_charger_store);

static struct attribute *asus_charger_attrs[] = {
	&dev_attr_demo_app_charger.attr,
	NULL,
};

static const struct attribute_group asus_charger_attr_group = {
	.attrs = asus_charger_attrs,
};
//asus bsp guochang_qiu demp app stop charger----

//guochang_qiu ATD_test++++
#ifdef ASUS_FACTORY_BUILD
static void charging_limit_check(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				asus_charging_limit_work.work);

	int capacity_now = get_prop_batt_capacity(chip);
	int recharge_cap = charging_limit_cap - 5;

	if(disable_charging == true){
		vote(chip->usb_suspend_votable, USER_EN_VOTER, true, 0);
		factory_limit_disable_charging = true;
		goto DIS_CHARGING ;
	}

	if ((charging_limit_cap < 10) || (capacity_now <= recharge_cap))
	{
		vote(chip->usb_suspend_votable, USER_EN_VOTER, false, 0);
		factory_limit_disable_charging = false;
		goto DIS_CHARGING ;
	}

	if (capacity_now >= charging_limit_cap)
	{
		vote(chip->usb_suspend_votable, USER_EN_VOTER, true, 0);
		factory_limit_disable_charging = true;
	}
...
DIS_CHARGING:
	schedule_delayed_work(&chip->asus_charging_limit_work,5*HZ);
	return;
}

ssize_t charging_limit_proc_write(struct file *file_chip, const char __user *buf,
						size_t len, loff_t *data)
{
	char messages[10];
	int limit_capacity = -1;
	...
	limit_capacity = (int)simple_strtol(messages, NULL, 10);
	if(limit_capacity == 1){
		limit_capacity = 65;
	}

	printk("charging_limit limit_capacity = %d\n", limit_capacity);

	if (limit_capacity >= 0 && limit_capacity <= 100 && limit_capacity != charging_limit_cap)
	{
		if(limit_capacity > charging_limit_cap)
			vote(smb_charger_dev->usb_suspend_votable, USER_EN_VOTER, false, 0);
		charging_limit_cap = limit_capacity;
		cancel_delayed_work(&smb_charger_dev->asus_charging_limit_work);
		schedule_delayed_work(&smb_charger_dev->asus_charging_limit_work,0);
	}

	return len;
}

ssize_t disable_charging_proc_write(struct file *file_chip, const char __user *buf,
						size_t len, loff_t *data)
{
	if(!smb_charger_dev){
		pr_err("smb_charger_dev is NULL!\n");
	...
	cancel_delayed_work(&smb_charger_dev->asus_charging_limit_work);
	schedule_delayed_work(&smb_charger_dev->asus_charging_limit_work, 0);
	pr_info("disable charging ok!\n");
	return len;
}

ssize_t enable_charging_proc_write(struct file *file_chip, const char __user *buf,
						size_t len, loff_t *data)
{
	if(!smb_charger_dev){
		pr_err("smb_charger_dev is NULL!\n");
	...
	cancel_delayed_work(&smb_charger_dev->asus_charging_limit_work);
	schedule_delayed_work(&smb_charger_dev->asus_charging_limit_work, 0);
	pr_info("enable charging ok!\n");
	return len;
}

static int charging_limit_proc_read(struct seq_file *buf, void *data)
{
	int ret = -1;

	ret = charging_limit_cap;
	seq_printf(buf, "charging_limit_cap:%d\n", ret);
...
static int chargerIC_status_proc_read(struct seq_file *buf, void *data)
{
	...
	if(!smb_charger_dev || !smb_charger_dev->chgr_base){
		pr_err("chip is NULL or addr is ZERO !\n");
		seq_printf(buf, "0\n");
		return rc;
	}

	rc = smbchg_read(smb_charger_dev, &value, smb_charger_dev->chgr_base + CHGR_STS, 1);
	if (rc) {
		seq_printf(buf, "0\n");
...
static int disable_charging_proc_read(struct seq_file *buf, void *data)
{
	if(true == disable_charging)
		seq_printf(buf, "1\n");
	else
		seq_printf(buf, "0\n");
	return 0;
...
static int enable_charging_proc_read(struct seq_file *buf, void *data)
{
	if(false == disable_charging)
		seq_printf(buf, "1\n");
	else
		seq_printf(buf, "0\n");
	return 0;
...
static int charging_limit_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, charging_limit_proc_read, NULL);
}
...
static int disable_charging_proc_open(struct inode *inode, struct  file *file)
{
	return single_open(file, disable_charging_proc_read, NULL);
}

static int enable_charging_proc_open(struct inode *inode, struct  file *file)
{
	return single_open(file, enable_charging_proc_read, NULL);
}

static const struct file_operations charging_limit_fops = {
	.owner = THIS_MODULE,
	.open = charging_limit_proc_open,
...
static const struct file_operations disable_charging_fops = {
	.owner = THIS_MODULE,
	.open = disable_charging_proc_open,
	.write = disable_charging_proc_write,
	.read = seq_read,
};

static const struct file_operations enable_charging_fops = {
	.owner = THIS_MODULE,
	.open = enable_charging_proc_open,
	.write = enable_charging_proc_write,
	.read = seq_read,
};

static void atd_test_battery_interface(struct smbchg_chip *chip)
{
	struct proc_dir_entry *chargerIC_status = NULL;
	struct proc_dir_entry *atd_disable_charging = NULL;
	struct proc_dir_entry *atd_enable_charging = NULL;
	struct proc_dir_entry *charging_limit = proc_create(charging_limit_PROC_FILE, 0666, NULL, &charging_limit_fops);
	if(!charging_limit){
		pr_err("creat charging_limit proc inode failed!\n");
		return ;
	}
	INIT_DELAYED_WORK(&chip->asus_charging_limit_work,charging_limit_check);
	schedule_delayed_work(&chip->asus_charging_limit_work, 10*HZ);

	chargerIC_status = proc_create(chargerIC_status_PROC_FILE, 0444, NULL, &chargerIC_status_fops);
	if(!chargerIC_status)
		pr_err("creat chargerIC_status proc inode failed!\n");

	atd_disable_charging = proc_create(disable_charging_PROC_FILE, 0666, NULL, &disable_charging_fops);
	if(!atd_disable_charging)
		pr_err("create atd_disable_charging proc inode failed!\n");

	atd_enable_charging = proc_create(enable_charging_PROC_FILE, 0666, NULL, &enable_charging_fops);
	if(!atd_enable_charging)
		pr_err("create atd_enable_charging proc inode failed!\n");
...
ssize_t cancel_stop_charging_proc_write(struct file *file_chip, const char __user *buf,
						size_t len, loff_t *data)
{
	char messages[10];
	...
	smb_charger_dev->cancel_stop_charging = (int)simple_strtol(messages, NULL, 10);

	printk("cancel_stop_charging = %d\n", smb_charger_dev->cancel_stop_charging);

	return len;
}

static int cancel_stop_charging_proc_read(struct seq_file *buf, void *data)
{
	if(!smb_charger_dev){
		pr_err("smb_charger_dev is NULL!\n");
		seq_printf(buf, "0\n");
...
static int cancel_stop_charging_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, cancel_stop_charging_proc_read, NULL);
}

static const struct file_operations cancel_stop_charging_fops = {
	.owner = THIS_MODULE,
	.open = cancel_stop_charging_proc_open,
	.write = cancel_stop_charging_proc_write,
	.read = seq_read,
};

#define cancel_stop_charging_PROC_FILE "driver/cancel_stop_charging"
static void asus_stop_charging_interface(struct smbchg_chip *chip)
{
	struct proc_dir_entry *cancel_stop_charging;
	if(!chip){
		pr_err("%s::chip is NULL!\n", __func__);
		return ;
	}
	chip->cancel_stop_charging = 0;

	cancel_stop_charging = proc_create(cancel_stop_charging_PROC_FILE, 0666, NULL, &cancel_stop_charging_fops);
	if(!cancel_stop_charging)
		pr_err("create cancel_stop_charging proc inode failed!\n");
...
#define FCC_400MA 		400
#define FCC_700MA 		700
#define FCC_600MA 		600
#define ASUS_FlOAT_4V34 		4340
#define ASUS_FlOAT_4V06 		4006
#define ASUS_FlOAT_4V20 		4200
#define ASUS_FlOAT_4V00 		4000
#define ASUS_FlOAT_4V38 		4380
#define ASUS_FlOAT_4V10 		4100
#define RCHG_100MV 		0
#define RCHG_200MV 		1

#define TCC_150MA 150
#define TCC_50MA  50
#define TCC_100MA  100
...
const int scorpio_jeita_fcc_cfg[]={700,700,2600,1880,900,1200,1200,1200};
const int gemini_jeita_fcc_cfg[]={600,600,2100,1200,600,1000,1000,1000};
...
static void asus_charge_config_sr(struct smbchg_chip *chip){
	switch(g_ASUS_PRJ_ID){
		...
		case PRJ_GEMINI_5W:
			...
			chip->asus_tb.fcc_cfg=FCC_600MA;
			chip->asus_tb.fv_cfg= ASUS_FlOAT_4V38;
			chip->asus_tb.rchg_cfg=RCHG_200MV;
			chip->asus_tb.tcc_cfg=TCC_100MA;
			chip->asus_tb.jeita_fcc_cfg=gemini_jeita_fcc_cfg;
			chip->asus_tb.jeita_fv_cfg=ASUS_FlOAT_4V10;
			break;
		case PRJ_SCORPIO_10W:
		case PRJ_SCORPIO_5W:
			...
			chip->asus_tb.rchg_cfg=RCHG_200MV;
			chip->asus_tb.tcc_cfg=TCC_100MA;
			chip->asus_tb.jeita_fcc_cfg=scorpio_jeita_fcc_cfg;
			chip->asus_tb.jeita_fv_cfg=ASUS_FlOAT_4V10;
			break;
		default:
			chip->asus_tb.pcc_cfg=PCC_200MA;
...
static int asus_jeita_judge_state(struct smbchg_chip *chip, int batt_tempr, int batt_volt)
{
	...
	{
		if (batt_tempr >= chip->asus_tb.jeita_rechg_table[2]) //470
		{
			result_state = old_state;
		}
	}
	...
	{
		if (batt_tempr >= chip->asus_tb.jeita_rechg_table[3]) //570
		{
			result_state = old_state;
		}
	}
...
extern int get_battery_state_of_charge(void);
...
static int asus_do_soft_jeita(struct smbchg_chip *chip)
{
	...
	batt_soc = get_battery_state_of_charge();
	...
	if (ret < 0)
	{
		dev_err(chip->dev,"Couldn't set jeita temp hard limit\n");
	}
	...
	if (ret < 0)
	{
		dev_err(chip->dev,"Couldn't set 0x10FA\n");
	}

	//update jeita state according  to batt_temp
	asus_jeita_judge_state(chip,batt_temp,batt_volt);

	switch (chip->soft_jeita_state) {
	case JEITA_STATE_RANGE_01: // <15
	...
	case JEITA_STATE_RANGE_02: //15~100
	...
	case JEITA_STATE_RANGE_03: //100~200
		charging_enable = true;
	...
	case JEITA_STATE_RANGE_04: //200~500
		if(batt_volt > 4200)
		{
			fcc_value = chip->asus_tb.jeita_fcc_cfg[4];
		}
	...
	case JEITA_STATE_RANGE_05: //500~600
	...
	printk("soft_jeita_state=%d,asus_do_soft_jeita voltage=%d,batt_float_volt=%d,temp=%d,batt_soc=%d,fcc = %d, float voltage = %d \n",
		chip->soft_jeita_state,batt_volt,batt_float_volt,batt_temp,batt_soc,fcc_value,float_volt);
	...
	#ifdef ASUS_FACTORY_BUILD
		if(is_usb_present(chip) == true && factory_limit_disable_charging == true)
		{
			goto skip_charging_enable;
		}
	#endif

	if(is_usb_present(chip) == true && demo_app_status_flag == true){
		goto skip_charging_enable;
	}
	...
	smbchg_read(chip,&reg,0x100E,0x1);
	if((batt_soc <= 98) && ((reg&0x20)==0x20) && chip->usb_present)
	{
		if((chip->soft_jeita_state >= JEITA_STATE_RANGE_02) &&(chip->soft_jeita_state <= JEITA_STATE_RANGE_04))
		{
			vote(chip->battchg_suspend_votable, BATTCHG_JEITA_EN_VOTER, true, 0);
			vote(chip->battchg_suspend_votable, BATTCHG_JEITA_EN_VOTER, false, 0);
		}
	}
	goto finish;
	skip_charging_enable:
		pr_info("skip_charging_enable \n");
...
void jeita_monitor_chg_temp(int64_t * chg_temp)
{
	int chg_temp_level,rc,capacity;
	...
	capacity = get_prop_batt_capacity(smb_charger_dev);

	mutex_lock(&smb_charger_dev->therm_lvl_lock);

	if(smb_charger_dev->cancel_stop_charging)
		goto out;
	...
	else if((*chg_temp < 65) && (*chg_temp >= 57))
	{
		chg_temp_level = 2;
	}
	else
	...
	pr_info("chg_temperature = %lld,capacity = %d , chg_temp_level = %d \n",*chg_temp,capacity,chg_temp_level);

	if (chg_temp_level == 3) //thermal level = 3
	{
		rc = vote(smb_charger_dev->usb_suspend_votable, THERMAL_EN_VOTER, true, 0);
		if (rc < 0)
		{
			dev_err(smb_charger_dev->dev,
					"Couldn't set usb suspend rc %d\n", rc);
			goto out;
		}
		goto out;
	}
	else if (chg_temp_level == 0 || chg_temp_level == 1) //thermal level = 0 or 1
	{
		rc = vote(smb_charger_dev->usb_suspend_votable, THERMAL_EN_VOTER,
									false, 0);
		if (rc < 0)
		{
			dev_err(smb_charger_dev->dev,
		...
		rc = vote(smb_charger_dev->usb_icl_votable, THERMAL_ICL_VOTER, false, 0);
		if (rc < 0)
			pr_err("Couldn't disable USB thermal ICL vote rc=%d\n",
					rc);

		rc = vote(smb_charger_dev->dc_icl_votable, THERMAL_ICL_VOTER, false, 0);
		if (rc < 0)
				pr_err("Couldn't disable DC thermal ICL vote rc=%d\n",
					rc);
	}
	else if (chg_temp_level == 2) //thermal level = 2
	{
		rc = vote(smb_charger_dev->usb_suspend_votable, THERMAL_EN_VOTER,
									false, 0);
		if (rc < 0)
		...
		if(capacity < 8)
		{
			rc = vote(smb_charger_dev->usb_icl_votable, THERMAL_ICL_VOTER, true,
						900);
			if (rc < 0)
				pr_err("Couldn't vote for USB thermal ICL rc=%d\n", rc);
		}
...
out:
	mutex_unlock(&smb_charger_dev->therm_lvl_lock);
...
static void asus_batt_temp_work(struct work_struct *work)
{
	...
	int ret;
	int64_t chg_temperature;
	struct timespec time_now;
	long delta_time;

	if(soft_jeita_disable == 0)
	{
	...
	if(is_usb_present(chip) == true && factory_limit_disable_charging == true)
	{
		goto skip_asus_do_soft_jeita;
	}
#endif

	time_now = current_kernel_time();
	delta_time = (long)(time_now.tv_sec - thermal_engine_control_time.tv_sec);
	pr_debug("delta time = %ld \n",delta_time);

	if(delta_time >= 30)
	{
		chg_temperature = get_chg_temp();
		jeita_monitor_chg_temp(&chg_temperature);
	}

#ifdef ASUS_FACTORY_BUILD
skip_asus_do_soft_jeita:
#endif
	...
	{
		pr_info("capacity is full,skip jeita rtc work \n");
		disable_jeita_alarm = true;
		schedule_delayed_work(&chip->asus_batt_temp_work,60*HZ);
	}
	pr_debug("finish asus_batt_temp_work \n");
...
static void asus_sdp_det_work(struct work_struct *work)
{
	...
	if(usb_present)
	{
		...
		ret = chip->usb_psy->get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &val);
		if (!ret)
			value = val.intval;
		pr_info("usb type => %d \n",value);
		...
		if(!chip->force_rerun_apsd)
		{
			...
			{
				chip->asus_charging_type = SDP;
				pr_info("first check USB type is not DCP,so it's not floating\n");
				goto out;
			}
		}
		else
		{
			if(value == 5)
			{
				pr_info("second check USB type is DCP,so it's floating\n");
				chip->asus_charging_type = FLOATING;
				...
				rc = vote(chip->usb_icl_votable, FLOATING_CHG_VOTER, true, 500);
				if (rc < 0) {
					dev_err(chip->dev, "Couldn't set usb_icl_votable rc=%d\n", rc);
				}
			...
			{
				chip->asus_charging_type = SDP;
				pr_info("second check USB type is not DCP,so it's not floating\n");
				chip->force_rerun_apsd=false;
				goto out;
			}
		}
...
static void asus_handler_usb_removal(struct smbchg_chip *chip)
{
	...
	//add uts USB plug-in/out log guochang_qiu +++
	if(0 == enable_uts_log)
		UTS_chg_mode(POWER_SUPPLY_TYPE_CABLE_OUT, chip);
	//add uts USB plug-in/out log guochang_qiu ---
	...
	vote(chip->usb_icl_votable, FLOATING_CHG_VOTER, false, 500);

	//chip->force_rerun_apsd=false;
	wake_unlock(&asus_soc_check_wake_lock);
	if (chip->psy_registered)
		power_supply_changed(&chip->batt_psy);
}
...
static void smbchg_pre_config(struct smbchg_chip *chip)
{
	...
	//6.set USB IN current limit 900mA
	...
	// 7.
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CFG1,
			SMB_MASK(7,0), BIT(6)|BIT(2)|BIT(0));
	...
	rc = smbchg_sec_masked_write(chip, chip->misc_base + 0xF5,
			SMB_MASK(7,0) ,BIT(7)|BIT(4)|BIT(3));
	if (rc < 0) {
...
struct seq_file *dump_buf;
static inline void dump_reg_now(struct smbchg_chip *chip, u16 addr,
		const char *name)
{
	u8 reg;
	...
	smbchg_read(chip, &reg, addr, 1);
	//printk("%s - %04X = %02X\n", name, addr, reg);
	seq_printf(dump_buf, "%s%04X = %02X\n", "", addr, reg);
}

/* dumps useful registers for debug */
static void dump_regs_now(struct smbchg_chip *chip)
{
	u16 addr;

	/* charger peripheral */
	for (addr = 0xB; addr <= 0x10; addr++)
		dump_reg_now(chip, chip->chgr_base + addr, "CHGR Status");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->chgr_base + addr, "CHGR Config");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->otg_base + addr, "OTG Reg");
	/* battery interface peripheral */
	dump_reg_now(chip, chip->bat_if_base + RT_STS, "BAT_IF Status");
	dump_reg_now(chip, chip->bat_if_base + CMD_CHG_REG, "BAT_IF Command");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->bat_if_base + addr, "BAT_IF Config");
	/* usb charge path peripheral */
	for (addr = 0x7; addr <= 0x10; addr++)
		dump_reg_now(chip, chip->usb_chgpth_base + addr, "USB Status");
	dump_reg_now(chip, chip->usb_chgpth_base + CMD_IL, "USB Command");
	dump_reg_now(chip, chip->usb_chgpth_base + 0x41, "USB Command");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->usb_chgpth_base + addr, "USB Config");
	/* dc charge path peripheral */
	dump_reg_now(chip, chip->dc_chgpth_base + RT_STS, "DC Status");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->dc_chgpth_base + addr, "DC Config");
	/* misc peripheral */
	dump_reg_now(chip, chip->misc_base + IDEV_STS, "MISC Status");
	dump_reg_now(chip, chip->misc_base + RT_STS, "MISC Status");
	for (addr = 0xF0; addr <= 0xFF; addr++)
		dump_reg_now(chip, chip->misc_base + addr, "MISC CFG");
}

static int smb_charger_reg_dump_proc_read(struct seq_file *buf, void *data)
{
	dump_buf = buf;
	dump_regs_now(smb_charger_dev);
	return 0;
}
...
#define SOFT_JEITA_DISABLE_PROC_FILE "driver/soft_jeita_disable"

ssize_t soft_jeita_disable_proc_write(struct file *file_chip, const char __user *buf,
						size_t len, loff_t *data)
{
	char messages[10];
	...
	soft_jeita_disable = (int)simple_strtol(messages, NULL, 10);
	printk("soft_jeita_disable:%d\n", soft_jeita_disable);
	return len;
}

static int soft_jeita_disable_proc_read(struct seq_file *buf, void *data)
{
	int ret = -1;

	ret = soft_jeita_disable;
	seq_printf(buf, "soft_jeita_disable:%d\n", ret);

	return 0;
}


static int soft_jeita_disable_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, soft_jeita_disable_proc_read, NULL);
}
static const struct file_operations soft_jeita_disable_fops = {
	.owner = THIS_MODULE,
	.open = soft_jeita_disable_proc_open,
	.write = soft_jeita_disable_proc_write,
	.read = seq_read,
};

static void create_soft_jeita_disable_proc_file(void)
{
	struct proc_dir_entry *soft_jeita_disable_proc_file = proc_create(SOFT_JEITA_DISABLE_PROC_FILE, 0666, NULL, &soft_jeita_disable_fops);

	if (soft_jeita_disable_proc_file) {
		printk("[BAT][CHG][SMB][Proc]SOFT_JEITA_DISABLE_PROC_FILE create ok!\n");
	} else{
		printk("[BAT][CHG][SMB][Proc]SOFT_JEITA_DISABLE_PROC_FILE create failed!\n");
	}
	soft_jeita_disable = 0;
}
...
void asus_smb_charger_init(struct smbchg_chip *chip)
{
	int rc;
	// 1. disable HVDCP
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + 0xF4,
				BIT(3),0);
	if (rc < 0) {
		pr_err("Couldn't disable HVDCP rc=%d\n", rc);
	}
	...
	// 6.config pre charge current (default)
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + 0xF1,
			BIT(2)|BIT(1)|BIT(0), chip->asus_tb.pcc_cfg);
...
//	printk("[BAT][alarm]Jeita_Alarm triggered\n");
	...
	{
		wake_lock_timeout(&asus_jeita_alarm_wake_lock,2 * HZ);
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	wake_lock_init(&asus_soc_check_wake_lock, WAKE_LOCK_SUSPEND, "asus_soc_check_wake");
	wake_lock_init(&asus_jeita_alarm_wake_lock, WAKE_LOCK_SUSPEND, "asus_jeita_alarm_wake");
	...
	//guochang_qiu bsp demp app stop charger +++
	INIT_DELAYED_WORK(&chip->demo_app_work, dmeo_app_control_charger);
	//guochang_qiu bsp demp app stop charger ---
	...
	if (chip->cfg_chg_led_support &&
		...
		chip->led_reg=regulator_get(&chip->spmi->dev,
							"led-power");
		if (IS_ERR(&chip->led_reg)) {
			rc = PTR_ERR(&chip->led_reg);
			dev_err(&chip->spmi->dev,
				"chg_led regulator get failed(%d)\n", rc);
			goto unregister_led_class;
		}
		chip->led_reg_enable=0;

		if(chip->led_reg){
			OF_PROP_READ(chip, chip->led_reg->max_uV, "led-power-max-voltage", rc, 1);
			OF_PROP_READ(chip, chip->led_reg->min_uV, "led-power-min-voltage", rc, 1);
		}
	}
	...
	chip->soft_jeita_state = JEITA_STATE_INITIAL;
	...
	create_soft_jeita_disable_proc_file();
	...
	//asus report capacity frank ++++
	asus_polling_battery_data_work(10);
	//asus report capacity frank ----

	//guochang_qiu ATD_test++++
#ifdef ASUS_FACTORY_BUILD
	atd_test_battery_interface(chip);
#endif
	//guochang_qiu ATD_test ----

	asus_stop_charging_interface(chip);

	//guochang_qiu bsp demp app stop charger +++
	rc = sysfs_create_group(&chip->dev->kobj, &asus_charger_attr_group);
	if(rc < 0){
		dev_err(&spmi->dev, "create sysfs group failed = %d\n", rc);
		goto exit_remove;
	}
	//guochang_qiu bsp demp app stop charger ---
...
exit_remove:
	sysfs_remove_group(&chip->dev->kobj, &asus_charger_attr_group);

unregister_led_class:
	if (chip->led_reg)
		regulator_put(chip->led_reg);
...
static int smbchg_remove(struct spmi_device *spmi)
{
	...
	wake_lock_destroy(&asus_soc_check_wake_lock);
	wake_lock_destroy(&asus_jeita_alarm_wake_lock);
...
static int smbchg_resume(struct device *dev)
{
	...
	if(get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_CHARGING && disable_jeita_alarm == true)
	{
		disable_jeita_alarm = false;
		schedule_delayed_work(&SetJeitaRTCWorker, 0);
	}
