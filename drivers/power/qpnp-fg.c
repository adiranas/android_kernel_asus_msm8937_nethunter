...
//guochang_qiu ATD Test gaugeIC_status+++++
...
static int gaugeIC_status_proc_read(struct seq_file *buf, void *v)
{
	...
	if(!FG_stats_chip || !FG_stats_chip->mem_base){
		pr_err("FG_stats_chip is NULL or addr is ZERO !\n");
		seq_printf(buf, "0\n");
		return rc;
	}

	rc = fg_read(FG_stats_chip, &value, (FG_stats_chip->mem_base+4), 1);
	if (rc) {
		seq_printf(buf, "0\n");
...
static void ATD_test_get_gaugeIC_status(struct fg_chip *chip)
{
...
{
	int ret,j,current_now;
	u8 reg[4];
	int64_t temp;

	if(!bms_chip){
		pr_err("bms_chip is NULL!\n");
		return -1;
	}

	fg_mem_lock(bms_chip);

	ret = fg_mem_read(bms_chip, reg, 0x5CC, 2, 3, 0);
	if (ret)
	{
		pr_err("Failed to read fg current data\n");
		return -1;;
	}
...
static int ATD_battery_voltage_proc_read(struct seq_file *buf, void *data)
{
	...
	ret = fg_mem_read(bms_chip, reg, 0x5CC, 2, 1, 0);
	if (ret)
	{
		pr_err("Failed to read fg voltage data\n");
		return -1;;
	}
...
{
	int battery_soc;
	battery_soc = get_battery_soc_raw(bms_chip) * 100 / FULL_PERCENT_3B;
	return battery_soc;
}
...
static ssize_t batt_switch_name(struct switch_dev *sdev, char *buf)
{
	...
	char battery_id[2] = "00";
	char profile_version[5] = "0001";
	char sw_driver_version[10] = "1.1.10.34";
	...
	if (!psy)
	{
		pr_err("bms supply not found. defer\n");
	}
	...
	if(g_ASUS_PRJ_ID == PRJ_SCORPIO_5W || g_ASUS_PRJ_ID == PRJ_SCORPIO_10W)
	{
		if(batt_id_kohm < 57 && batt_id_kohm > 42)
		{
			Cell_Supplier_Code = 'O';
			strcpy(battery_id,"01");
		}
		else
		if(batt_id_kohm < 12 && batt_id_kohm > 8)
		{
			Cell_Supplier_Code = 'A';
			strcpy(battery_id,"02");
		}
		else
		{
			pr_info("Unknow battery \n");
		}
		strcpy(Battery_Model_Name,"C11P1606");
		strcpy(profile_version,"0001");
	}
	else
	if(g_ASUS_PRJ_ID == PRJ_GEMINI_5W || g_ASUS_PRJ_ID == PRJ_GEMINI_10W)
	{
		if(batt_id_kohm < 57 && batt_id_kohm > 42)
		{
			Cell_Supplier_Code = 'T';
			strcpy(battery_id,"01");
		}
		else
		{
		...
		strcpy(Battery_Model_Name,"C11P1607");
		strcpy(profile_version,"0001");
	}
	else
	{
		pr_info("Unknow project \n");
	}
	pr_info("%s-%c-%s-%s-%s\n", Battery_Model_Name, Cell_Supplier_Code, battery_id, profile_version, sw_driver_version);
	return sprintf(buf, "%s-%c-%s-%s-%s\n", Battery_Model_Name, Cell_Supplier_Code, battery_id, profile_version, sw_driver_version);
}
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	//guochang_qiu ATD Test FG status++++
#ifdef ASUS_FACTORY_BUILD
	ATD_test_get_gaugeIC_status(chip);
