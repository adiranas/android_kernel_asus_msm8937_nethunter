...
////ASUYS_BSP:remove focal touch driver
//extern void ftxxxx_ts_suspend(void);
//extern void ftxxxx_ts_resume(void);
extern  void goodix_ts_suspend(void);
extern  void goodix_ts_resume(void);
int focaltp ;
...
static char read_panel_reg[1] = {0x0A};

static struct dsi_cmd_desc read_reg_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,1},read_panel_reg},
};

static struct proc_dir_entry *read_panel_proc;
void read_panel_register(char *rbuf);
static ssize_t read_panel_register_proc_write(struct file *file, const const char __user *buff, size_t count,loff_t *ops)
{
	int val = simple_strtoul(buff, NULL, 16);
	read_panel_reg[0] = (char)val;

	return count;
}

static int read_panel_register_proc_read(struct seq_file *buf, void *v)
{
		char temp[2] = {0};
		read_panel_register(&temp[0]);

		seq_printf(buf, "reg 0x%x:0x%x\n", read_panel_reg[0], temp[0]);

		return 0;
}

static int read_panel_register_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, read_panel_register_proc_read,NULL);
}

static struct file_operations read_panel_register_proc_ops = {
	.open = read_panel_register_proc_open,
	.read = seq_read,
	.write = read_panel_register_proc_write,
	.release = single_release,
};
static void create_read_panel_register_file(void)
{
	printk("[DISPLAY] : create_read_panel_register_file\n");
	read_panel_proc = proc_create("read_panel_register", 0666, NULL, &read_panel_register_proc_ops);
	if(read_panel_proc){
        printk("[DISPLAY] : create_read_panel_register_file sucessed!\n");
    }else{
		printk("[DISPLAY] : create read_panel_register failed!\n");
    }
}

//ASUS_BSP: show_wang ++++ add cabc function for Scorpio
#include <linux/workqueue.h>
#define PANEL_CABC_MASK	0x3
char dimming_cmd[2] = {0x53, 0x2C};
char bl_cmd[2] = {0x51, 0x64};
char bl_cmd1[2] = {0x51, 0xFF};
char bl_cmd2[3] = {0x51, 0x0F, 0xFF};
...
static void bl_calculate(struct dcs_cmd_req *cmdreq)
{
    ...
    if (g_asus_lcdID == ZC551KL_LCD_CTC/* || g_asus_lcdID == ZC551KL_LCD_BOE*/) {
        max = 4095;
...
int support_cabc(void) {
	//return 0; //remove cabc function temporary for flash screen
	if( g_asus_lcdID == ZC551KL_LCD_CTC|| g_asus_lcdID == ZC551KL_LCD_TM ) {
		return 1;
	} else {
		return 0;
	}
}
EXPORT_SYMBOL(support_cabc);
...
//ASUS_BSP: show_wang +++ add proc node to send lcd_uniqueID
...
//ASUS_BSP: show_wang +++ add proc node to send lcd_ID
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
    ...
    if (level < pinfo->bl_min)
        level = pinfo->bl_min;
	if(support_cabc()) {
		if ((g_asus_lcdID == ZC551KL_LCD_CTC /*|| g_asus_lcdID == ZC551KL_LCD_BOE*/) && level <= ILI7807B_BL_THRESHOLD && level > 0)
			level =ILI7807B_BL_THRESHOLD;
		else if (g_asus_lcdID == ZC551KL_LCD_TM && level <= R63350_BL_THRESHOLD &&  level > 0)
			level =R63350_BL_THRESHOLD;
		//pr_debug("%s:[DISPLAY] level=%d \n", __func__, level);
	//ASUS_BSP: show_wang ++++ add cabc function for Scorpio
		if( g_asus_lcdID == ZC551KL_LCD_CTC/* || g_asus_lcdID == ZC551KL_LCD_BOE*/) {
			led_pwm2[1] = (unsigned char) (level >> (8 - bl_level_shift));
			led_pwm2[2] = (unsigned char) (level << bl_level_shift);

			pr_debug("%s::[DISPLAY]LCD_ID=%d level(%x) level(%x) \n", __func__, g_asus_lcdID, led_pwm2[1], led_pwm2[2]);
			memset(&cmdreq, 0, sizeof(cmdreq));
			...
			pr_debug("%s::[DISPLAY] LCD_ID=%d  level(%d)\n", __func__, g_asus_lcdID, level);
		//ASUS_BSP: show_wang ---- add cabc function for Scorpio
		...
		printk("%s::[DISPLAY] LCD_ID=%d  level(%d)\n", __func__, g_asus_lcdID, level);
		led_pwm1[1] = (unsigned char)level;
		memset(&cmdreq, 0, sizeof(cmdreq));
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		gpio_free(ctrl_pdata->rst_gpio);
		if( g_asus_lcdID == ZC551KL_LCD_TM )
...
//ASUS_BSP: show_wang ----add cabc function for Scorpio
	...
	case BL_WLED:
		pr_debug("[Display] %s:: BL_WLED level(%d)\n", __func__, bl_level);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if(focaltp==0)
	{
		goodix_ts_suspend();//suspend touch
	}
	//ASUYS_BSP:remove focal touch driver
	/*else{
		ftxxxx_ts_suspend();
...
EXPORT_SYMBOL(read_panel_register);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if(support_cabc()) {
		...
		while (bl_level_tmp%256 == 0 && (g_asus_lcdID == ZC551KL_LCD_CTC/* || g_asus_lcdID ==ZC551KL_LCD_BOE*/) ) {
			bl_level_tmp /= 2;
			bl_level_shift++;
			if (bl_level_tmp == 256) {
	...
	int ndx)
{
	...
	create_read_panel_register_file();
