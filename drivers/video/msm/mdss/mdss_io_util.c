...
int msm_dss_enable_vreg(struct dss_vreg *in_vreg, int num_vreg, int enable)
{
	...
	if (enable) {
		for (i = 0; i < num_vreg; i++) {
			...
			//[yudong] add for panel vreg debug +++
			pr_debug("[Display]%s enable vreg :%s, min:%d V, max:%d V \n",
			__func__,in_vreg[i].vreg_name, in_vreg[i].min_voltage, in_vreg[i].max_voltage);
		...
		for (i = num_vreg-1; i >= 0; i--) {
			...
			//[yudong] add for panel vreg debug +++
			pr_debug("[Display]%s disable vreg :%s, min:%d V, max:%d V \n",
			__func__,in_vreg[i].vreg_name, in_vreg[i].min_voltage, in_vreg[i].max_voltage);
