...
extern char bl_cmd1[2];
extern char bl_cmd2[3];
extern int support_cabc(void);
//ASUS_BSP: show_wang ----add cabc function for Scorpio
						...
						unsigned long *argp)
{
	...
	if(support_cabc()) {
		if (display_commit_cnt > 0) {
			if (display_commit_cnt == 1 && mdss_first_boot) {
				printk("[Display]  first boot set cabc[0](%x) = cabc[1](%x)\n", cabc_mode[0], cabc_mode[1]);
