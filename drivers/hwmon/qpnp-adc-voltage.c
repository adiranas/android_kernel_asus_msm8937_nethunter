...
static struct qpnp_vadc_scale_fn vadc_scale_fn[] = {
	...
	[ASUS_SCALE_SMBCHG_TEMP] = {asus_qpnp_adc_scale_smbchg_temp},//add bsp guochang_qiu
};
...
int64_t get_chg_temp(void)
{
	...
	if(vadc_dev)
	{
		...
		{
			if (rc != -EPROBE_DEFER)
			pr_err("VADC SMBCHG_TEMP read error with %d\n", rc);
			return rc;
		}
		temp = result.physical;
	}
