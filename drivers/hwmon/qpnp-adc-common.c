...
//add bsp guochang_qiu for smbchg chan+++
/* Voltage to temperature */
static const struct qpnp_vadc_map_pt adcmap_smbchg[] = {
	{1758, -40},
	{1742, -35},
	{1719, -30},
	{1691, -25},
...
//add bsp guochang_qiu for smbchg chan+++
int32_t asus_qpnp_adc_scale_smbchg_temp(struct qpnp_vadc_chip *vadc,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	int64_t smbchg_voltage = 0;
	...
	if (adc_properties->adc_hc) {
		/* (ADC code * vref_vadc (1.875V) * 1000) / (0x4000 * 1000) */
		smbchg_voltage = (int64_t) adc_code;
		smbchg_voltage *= (int64_t) (adc_properties->adc_vdd_reference
							* 1000);
		smbchg_voltage = div64_s64(smbchg_voltage,
					QPNP_VADC_HC_VREF_CODE * 1000);
		qpnp_adc_map_voltage_temp(adcmap_smbchg,
			ARRAY_SIZE(adcmap_smbchg),
			smbchg_voltage, &adc_chan_result->physical);
	} else {
		qpnp_adc_scale_with_calib_param(adc_code,
			adc_properties, chan_properties, &smbchg_voltage);

		if (chan_properties->calib_type == CALIB_ABSOLUTE)
			do_div(smbchg_voltage, 1000);
		pr_debug("smbchg_voltage=%lld\n", smbchg_voltage);
		qpnp_adc_map_voltage_temp(adcmap_smbchg,
			ARRAY_SIZE(adcmap_smbchg),
			smbchg_voltage, &adc_chan_result->physical);
	}
...
EXPORT_SYMBOL(asus_qpnp_adc_scale_smbchg_temp);
//add bsp guochang_qiu for smbchg chan---
