...
struct delayed_work shutdown_cpu_wq;
static bool qeneue=0;
static void shutdown_cpu(struct work_struct *work)
{
	int i = 0;
	for(i=1;i<8;i ++)
	{
		printk("[power]in charge mode shutdown cpu%d\n",i);
		cpu_down(i);
	}
}
...
int register_cpu(struct cpu *cpu, int num)
{
	...
	if(qeneue ==0 && g_CHG_mode == 1)
	{
		schedule_delayed_work(&shutdown_cpu_wq,60*HZ);
		qeneue=1;
	}
	return error;
}
...
void __init cpu_dev_init(void)
{
	...
	INIT_DELAYED_WORK(&shutdown_cpu_wq,shutdown_cpu);
