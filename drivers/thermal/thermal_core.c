...
//+++ ASUS_BSP gauss_li for thermal factory test
#ifdef ASUS_FACTORY_BUILD
static ssize_t
atd_temp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct thermal_zone_device *tz = to_thermal_zone(dev);
	long temperature;
	int ret;
    ...
    if(g_ASUS_PRJ_CPUTYPE==1 && tz->id != 0 && tz->id != 1 && tz->id != 18)   //this tz node temperature unit has  (mc)
		temperature *= 1000;
	else if(g_ASUS_PRJ_CPUTYPE==0 && tz->id != 0 && tz->id != 1 && tz->id != 17)  //this tz node temperature unit has  (mc)
		temperature *= 1000;
	return sprintf(buf, "%ld\n", temperature);
}
#endif
...
//+++ ASUS_BSP gauss_li for thermal factory test
#ifdef ASUS_FACTORY_BUILD
static DEVICE_ATTR(atdtemp, 0444, atd_temp_show, NULL);
#endif
//--- ASUS_BSP gauss_li for thermal factory test
...
//+++ ASUS_BSP gauss_li for thermal factory test
#ifdef ASUS_FACTORY_BUILD
    result = device_create_file(&tz->device, &dev_attr_atdtemp);
	if (result)
		goto unregister;
#endif
//--- ASUS_BSP gauss_li for thermal factory test
