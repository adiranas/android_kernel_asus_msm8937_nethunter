...
//ASUS_BSP : Freeman +++
static ssize_t
testmode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "Test mode setting :\n"
			"0: Test mode disable\n"
			"1: Test J mode\n"
			"2: Test K mode\n"
			"3: Test SE0 NAK mode\n"
			"4: Test packet mode\n"
			"5: Test mode force enable\n"
			"6: RESET\n"
			"7: SUSPEND\n"
			"8: RESUME\n");
}
extern int set_port_feature(struct usb_device *hdev, int port1, int feature);
static ssize_t testmode_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct usb_device *udev = to_usb_device(dev);
	int             status;
	...
	sscanf(buf, "%d %d", &mode, &port);
	printk(KERN_ALERT "testmode = %d, port = %d ``````````\n",
			mode, port);

	port1 = mode << 8 | port;
	printk(KERN_ALERT "####### port1 = %#x\n", port1);
	switch(mode){
		case 0: // TESTMODE disable
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			break;
		case 1: // Test J mode
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			break;
		case 2: // Test K mode
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			break;
		case 3: // Test SE0 NAK mode
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			break;
		case 4: // Test Packet mode
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			printk(KERN_ALERT "####### status = %d\n", status);
			if (status >= 0)
				printk(KERN_ALERT "####### Set Test Packet mode success.\n");
			else
				printk(KERN_ALERT "####### Set Test Packet mode failed.\n");
			break;
		case 5: // Test force eanble
			status = set_port_feature(udev, port1, USB_PORT_FEAT_TEST);
			break;
		case 6: // RESET
			status = set_port_feature(udev, 1, USB_PORT_FEAT_RESET);
			break;
#if 0
		case 7: // SUSPEND
			udev->autosuspend_disabled = 0;
			udev->autoresume_disabled = 1;
			status = usb_external_suspend_device(udev, PMSG_USER_SUSPEND);
			break;
		case 8: // RESUME
			udev->autosuspend_disabled = 1;
			udev->autoresume_disabled = 0;
			status = usb_external_resume_device(udev, PMSG_USER_RESUME);
			break;
#endif
		default:
			break;
...
//ASUS_BSP : Freeman +++
	sysfs_attr_init(&dev->usb_eye_test.attr);
	dev->usb_eye_test.attr.name = "testmode";
	dev->usb_eye_test.attr.mode = S_IRUGO | S_IWUGO;
	dev->usb_eye_test.show = testmode_show;
	dev->usb_eye_test.store = testmode_store;
//ASUS_BSP : Freeman ---
...
//ASUS_BSP : Freeman +++
	retval = sysfs_add_file_to_group(&dev->kobj, &dev->usb_eye_test.attr, power_group_name);
	if (retval)
	   goto error;
//ASUS_BSP : Freeman ---
