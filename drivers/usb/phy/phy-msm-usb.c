...
//ASUS_BSP : Freeman add [ZC600KL][USBH][NA][Spec] Enable manual mode switching
...
//ASUS_BSP : Freeman add [ZC600KL][USBH][NA][fix] Add mutex to protect suspend/resume function
static struct mutex msm_otg_mutex;
//ASUS_BSP : Freeman add [ZC600KL][USBH][NA][fix] Add mutex to protect suspend/resume function

//ASUS_BSP : Freeman [ZC600KL][USBH][NA][Spec] Add dynamic setting support for phy parameters
...
//ASUS_BSP : Freeman [ZC600KL][USBH][NA][other] Add debug file for checking VBUS output status
static bool g_vbus_is_on = false;
//ASUS_BSP : Freeman [ZC600KL][USBH][NA][other] Add debug file for checking VBUS output status
extern int asus_set_otg(bool toggle);

//ASUS_BSP : Freeman [ZC600KL][USBH][NA][Spec] +++
...
//ASUS_BSP : Freeman [ZC600KL][USBH][NA][Spec] Enable manual mode switching
...
static void asus_otg_mode_switch(enum usb_mode_type req_mode)
{
	...
	case USB_HOST:
		...
		motg->host_mode = true;
//ASUS_BSP : Freeman [ZC600KL][USBH][Spec] Register early suspend notification for none mode switch
...
//ASUS_BSP : Freeman [ZC600KL][USBH][Spec] Add pm_suspended judgement to avoid system crash
...
//ASUS_BSP : Freeman [ZC600KL][USBH][Spec] Enable manual mode switching
...
//ASUS_BSP : Freeman [ZC600KL][USBH][NA][Spec] Add dynamic setting support for phy parameters
	...
	if (override_phy_init) {
		printk("[usb_otg] %s(): HUSB PHY Init:%s\n", __func__,
				override_phy_init);
...
//ASUS_BSP : Freeman [ZC600KL][USBH][Spec] Add mutex to protect suspend/resume function
...
//	printk("[usb_otg] %s(%s)\n", __func__, chg_to_string(motg->chg_type));
	if (motg->chg_type == USB_SDP_CHARGER){
		...
		//printk("[usb_otg] CABLE_OUT\n");
		asus_otg_set_charger(CABLE_OUT);
	}else{
		charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
		//printk("[usb_otg] unknown type\n");
...
//ASUS_BSP Freeman :[Z600KL][USBH] Enable/Disable VBUS output on ID events
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
#if 0
	int ret;
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	msleep(500);//ASUS_BSP Freeman sleep a litter to wait for the power supply for otg device
...
int is_msm_otg_in_host_state(void)
{
   struct usb_otg* otg = the_msm_otg->phy.otg;
   pr_debug("get otg state is %d -> %s\n",otg->phy->state,usb_otg_state_string(otg->phy->state));
   return (otg->phy->state == OTG_STATE_A_HOST);
}
EXPORT_SYMBOL_GPL(is_msm_otg_in_host_state);
...
//ASUS_BSP : Freeman [ZC600KL][USBH][NA][Spec] Add dynamic setting support for phy parameters
	msm_otg_dentry = debugfs_create_file("phy_parameter_a", S_IRUGO |
		S_IWUGO, msm_otg_dbg_root, motg, &msm_otg_phy_parameter_a_fops);
	...
	msm_otg_dentry = debugfs_create_file("phy_parameter_b", S_IRUGO |
		S_IWUGO, msm_otg_dbg_root, motg, &msm_otg_phy_parameter_b_fops);
	...
	msm_otg_dentry = debugfs_create_file("phy_parameter_c", S_IRUGO |
		S_IWUGO, msm_otg_dbg_root, motg, &msm_otg_phy_parameter_c_fops);
	...
	msm_otg_dentry = debugfs_create_file("phy_parameter_d", S_IRUGO |
		S_IWUGO, msm_otg_dbg_root, motg, &msm_otg_phy_parameter_d_fops);
