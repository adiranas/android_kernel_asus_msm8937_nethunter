...
#define	FLASH_LED_FAULT_DETECTED				0xE// 1 1 1 1 //ASUS_BSP Zhengwei_Cai "Fix for Zenflash1"
...
//ASUS_BSP +++ Zhengwei_Cai "Fix for Zenflash1"
static struct qpnp_flash_led *g_led_ctrl = NULL;
static uint8_t g_flash_self_check_en = 1;//default is 1
//ASUS_BSP --- Zhengwei_Cai "Fix for Zenflash1"
...
extern int zenflash_fired;
extern int zenflash_off;
static void qpnp_flash_led_work(struct work_struct *work)
{
	...
	pr_info("[ASUS ZENFLASH] flash node %s %s process....\n",
		flash_node->type == TORCH ? "TORCH":(flash_node->type == FLASH ? "FLASH":"SWITCH"),
		flash_node->id == FLASH_LED_0 ? "LED_1":(flash_node->id ==FLASH_LED_SWITCH ? "SWITCH":"LED_2")
	);
...
turn_off:
	...
	if (flash_node->type == TORCH) {
		...
		led->open_fault |= (val & FLASH_LED_FAULT_DETECTED);//ASUS_BSP Zhengwei_Cai "Fix for Zenflash1"
	}
...
//ASUS_BSP +++ Zhengwei_Cai "Fix for Zenflash1"
static int qpnp_flash_self_check_enable(struct qpnp_flash_led *led,int enable)
{
#if 1
	int rc;
	uint8_t val;
	if(enable)
		...
		val = FLASH_LED_DISABLE;
	pr_info("Zenflash1, FLASH_FAULT_DETECT write value 0x%x\n",val);
	rc = qpnp_led_masked_write(led->spmi_dev,
					FLASH_FAULT_DETECT(led->base),
						FLASH_FAULT_DETECT_MASK, val);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
					"Zenflash1, Fault detect reg write failed\n");
		return rc;
	}
#if 0
	rc = qpnp_led_masked_write(led->spmi_dev,
	...
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"Zenflash1, Secure reg write failed\n");
		return -EINVAL;
	}
	...
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"Zenflash1,failed to reset OTST2_RB bit\n");
		return rc;
	}
#endif
	return 0;
...
led->pdata->self_check_en = enable;

return qpnp_flash_led_init_settings(led);
...
static int self_check_enable_proc_read(struct seq_file *buf, void *v)
{
	int rc;
	uint8_t val;
	mutex_lock(&g_led_ctrl->flash_led_lock);
	rc = spmi_ext_register_readl(g_led_ctrl->spmi_dev->ctrl,
			g_led_ctrl->spmi_dev->sid,
			FLASH_FAULT_DETECT(g_led_ctrl->base),
			&val, 1);
	if(rc)
	{
		dev_err(&g_led_ctrl->spmi_dev->dev,
		"Unable to read from addr= %x, rc(%d)\n",
		FLASH_LED_FAULT_STATUS(g_led_ctrl->base), rc);
		seq_printf(buf, "fault status can not be read\n");
	}
	else
	{
		seq_printf(buf, "fault detect status is 0x%hhx\n",val);
	}
	mutex_unlock(&g_led_ctrl->flash_led_lock);
	return 0;
}

static int self_check_enable_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, self_check_enable_proc_read, NULL);
}

static ssize_t self_check_enable_proc_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	ssize_t ret_len;
	char messages[16]="";
	uint8_t val;
	...
	{
		case 0:
			rc = qpnp_flash_self_check_enable(g_led_ctrl,0);
			break;
		case 1:
			rc = qpnp_flash_self_check_enable(g_led_ctrl,1);
			break;
		default:
	...
	if(rc == 0)
	{
		pr_info("Zenflash1, flash self check %s!\n",val?"ENABLED":"DISABLED");
		g_flash_self_check_en = val;
		...
		if(rc)
		{
			dev_err(&g_led_ctrl->spmi_dev->dev,
			"Unable to read from addr= %x, rc(%d)\n",
			FLASH_FAULT_DETECT(g_led_ctrl->base), rc);
		}
		else
		{
			pr_info("Zenflash1, flash fault detect reg val is 0x%hhx now\n",val);
		}
	}
	else
	{
		pr_err("Zenflash1, flash self check write failed! current state %d\n",g_flash_self_check_en);
	}
	mutex_unlock(&g_led_ctrl->flash_led_lock);
	return ret_len;
}

static const struct file_operations self_check_enable_fops = {
	.owner = THIS_MODULE,
	.open = self_check_enable_proc_open,
	.read = seq_read,
	.write = self_check_enable_proc_write,
	.llseek = seq_lseek,
...
//ASUS_BSP +++ Zhengwei_Cai "Fix for Zenflash1"
	...
	proc_fd = proc_create("driver/asus_flash_self_check_en", 0666, NULL, &self_check_enable_fops);
	if(proc_fd)
	{
		pr_info("create flash_self_check_en succeeded!\n");
	}
	else
	{
		pr_err("create flash_self_check_en failed!\n");
	}
//ASUS_BSP --- Zhengwei_Cai "Fix for Zenflash1"
	return 0;
