...
#define LED_MPP_CURRENT_ASUS    25
		...
		struct device_node *node)
{
	...
	if(PRJ_SCORPIO_5W == g_ASUS_PRJ_ID || PRJ_SCORPIO_10W == g_ASUS_PRJ_ID){
		asus_gpio_high = gpio_get_value(led->asus_gpio);
		printk("[LED] asus_gpio_high=%d\n", asus_gpio_high);
		if(!asus_gpio_high)
			led->mpp_cfg->current_setting = LED_MPP_CURRENT_ASUS;
...
static int qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		//add by qiuguochang	++++
		if((g_ASUS_PRJ_ID == PRJ_SCORPIO_5W || g_ASUS_PRJ_ID == PRJ_SCORPIO_10W)&&g_ASUS_PRJ_CPUTYPE == CPUTYPE_MSM8937){ //richard +++ for 8917 portting
			led_pinctl = devm_pinctrl_get(&spmi->dev);
			if(!led_pinctl){
				dev_err(&spmi->dev, "Failure get led gpio pinctrl !\n");
				goto fail_id_check;
			}

			set_state = pinctrl_lookup_state(led_pinctl, "asus_led_gpio");
			if(!set_state){
				dev_err(&spmi->dev, "Failure lookup led gpio pinctrl state!\n");
				goto fail_id_check;
			}

			pinctrl_select_state(led_pinctl, set_state);
			led->asus_gpio = of_get_named_gpio(node, "qcom,asus-green-gpio", 0);
			if(!gpio_is_valid(led->asus_gpio)){
				dev_err(&led->spmi_dev->dev,"asus green gpio is not valid!\n");
				goto fail_id_check;
			}

			gpio_direction_input(led->asus_gpio);
		}
		//add by qiuguochang	----
