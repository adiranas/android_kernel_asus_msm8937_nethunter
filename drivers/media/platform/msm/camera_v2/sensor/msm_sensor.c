...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(rc==0){
		isSensorPowerup=1;
		s_camera_id = s_ctrl->id;
		msm_debugfs_set_ctrl(s_ctrl);
		if(s_ctrl->id == 0){
			g_main_ctrl = s_ctrl;
		}
	}
...
int s_sensor_read_temp(uint16_t *tmp)
{
	struct msm_camera_i2c_client *sensor_i2c_client;
	int rc = 0;
			...
			if(sensor_i2c_client != NULL){
				...
				if(val/1000000000LL >= 5){
					if(s_camera_id == 0){
						...
						temp = 157;
						pr_err("Stimber %s: read reg failed1\n", __func__);
					}
					if (rc < 0) {
						temp = 157;
						pr_err("Stimber %s: read reg failed2\n", __func__);
