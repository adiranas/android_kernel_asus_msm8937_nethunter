	...
	struct msm_actuator_set_position_t *set_pos)
{
	...
	for (index = 0; index < set_pos->number_of_steps; index++) {
		...
		pr_debug("Camera, g_vcm_position changed to %d\n",g_vcm_position);
		a_ctrl->i2c_tbl_index = 0;
	}
	CDBG("%s exit %d\n", __func__, __LINE__);
	return rc;
...
static int vcm_position_proc_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n", g_vcm_position);
	return 0;
}

static int vcm_position_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, vcm_position_proc_read, NULL);
}

static const struct file_operations vcm_position_fops = {
	.owner = THIS_MODULE,
	.open = vcm_position_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static void create_vcm_position_proc_file(void)
{
	struct proc_dir_entry *fd;
	static unsigned char iscreated=0;
	if(!iscreated){
    fd = proc_create(VCM_PROC_POSITION_FILE,0666, NULL, &vcm_position_fops);
		if(fd) {
			CDBG("Camera: %s sucessed!\n", __func__);
			iscreated = 1;
		} else {
...
#if 0  //ASUS_BSP remove unused log
