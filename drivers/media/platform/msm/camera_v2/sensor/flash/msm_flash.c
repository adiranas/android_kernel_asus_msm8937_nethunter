...
static struct msm_flash_ctrl_t *flash_ctrl = NULL;
static struct v4l2_file_operations msm_flash_v4l2_subdev_fops;
static struct led_trigger *torch_trigger;
extern int is_msm_otg_in_host_state(void);
...
static asus_flash_function_table_t flash_control_table;
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	asus_flash_set_led_state(MSM_CAMERA_LED_OFF);
	CDBG("Exit\n");
	return 0;
}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	CDBG("FLASH, msm_flash_init, flash_ctrl->flash_driver_type is %d\n",flash_ctrl->flash_driver_type);
	...
	CDBG("FLASH, msm_flash_init, flash_driver_type now is %d\n",flash_driver_type);
	...
	struct msm_flash_cfg_data_t *flash_data)
{
		...
		if (flash_ctrl->torch_trigger[i]) {
			...
			else
			{
				...
				if (flash_data->flash_current[i] >= 0 &&
					...
					curr = max_current;
					pr_debug("USB Host mode, LED current clamped to %d\n",
						curr);
				}
			}
	...
	asus_flash_set_led_state(MSM_CAMERA_LED_LOW);
	CDBG("Exit\n");
	return 0;
}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	for (i = 0; i < flash_ctrl->flash_num_sources; i++) {
		if (flash_ctrl->flash_trigger[i]) {
			...
			pr_info("high_flash_current[%d] = %d, max_flash_current is %d\n", i, curr,flash_ctrl->flash_max_current[i]);
	...
	if (flash_ctrl->switch_trigger)
		led_trigger_event(flash_ctrl->switch_trigger, 1);
	asus_flash_set_led_state(MSM_CAMERA_LED_HIGH);
	return 0;
}
	...
	struct msm_flash_ctrl_t *flash_ctrl)
{
	...
	asus_flash_set_led_state(MSM_CAMERA_LED_RELEASE);
	return 0;
}
	...
	void __user *argp)
{
	...
	case CFG_FLASH_RELEASE:
		...
		{
			asus_flash_lock();
			if(!asus_flash_get_flash_light_state())
			{
			...
			{
				pr_info("#### FlashLight ON, not release Flash ####\n");
				rc = 0;
			}
			asus_flash_unlock();
			asus_flash_set_camera_state(0);
		}
		break;
	case CFG_FLASH_OFF:
		if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)
		{
			...
			{
				pr_info("#### FlashLight ON, not turn off Flash ####\n");
				rc = 0;
			}
			asus_flash_unlock();
		}
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	if (of_get_property(of_node, "qcom,flash-source", &count)) {
		CDBG("count=%d after of_get_property\n", count);
		...
		for (i = 0; i < count; i++) {
			...
			if(fctrl->flash_max_current[i]>800)
			{
				CDBG("FLASH, flash %d max-current %d is too high, set to 800mA\n",i,fctrl->flash_max_current[i]);
				fctrl->flash_max_current[i] = 800;
			}
			if (rc < 0) {
				pr_err("current: read failed\n");
				of_node_put(flash_src_node);
	...
	if (of_get_property(of_node, "qcom,torch-source", &count)) {
		...
		for (i = 0; i < count; i++) {
			...
			CDBG("FLASH, torch %d max-current is %d\n",i,fctrl->torch_max_current[i]);
			if (rc < 0) {
				pr_err("current: read failed\n");
				of_node_put(torch_src_node);
				continue;
...
static int asus_led_init(void)
{
	int rc;
	flash_ctrl->func_tbl = &flash_table[2]->func_tbl;
	if (flash_ctrl->func_tbl->camera_flash_init) //NULL in PMIC
	{
		rc = flash_ctrl->func_tbl->camera_flash_init(
				flash_ctrl, NULL);
		if (rc < 0) {
	...
	flash_ctrl->flash_state = MSM_CAMERA_FLASH_INIT;
	asus_flash_set_led_state(MSM_CAMERA_LED_INIT);
	return rc;
}

static int asus_led_off(void)
{
	int rc;
	rc = flash_ctrl->func_tbl->camera_flash_off(flash_ctrl, NULL);
	if (rc < 0)
	...
	{
		//set state inside camera_flash_off
	}
	return rc;
}

static int asus_led_release(void)
{
	int rc;
	rc = flash_ctrl->func_tbl->camera_flash_release(flash_ctrl);
	if (rc < 0)
	{
		pr_err("%s:%d camera_flash_off failed rc = %d",
			__func__, __LINE__, rc);
	...
	{
		//set state inside camera_flash_release
	}
	return rc;
}

static int asus_led_flash(void)
{
	//int i;
	/* Turn off torch triggers */
	/*for (i = 0; i < flash_ctrl->torch_num_sources; i++)
...
static int asus_led_torch(void)
{
	//int i;
...
int asus_led_dual_flash(int current1, int current2)
{
	...
	cfg.flash_current[0] = current1;
	cfg.flash_current[1] = current2;

	return flash_ctrl->func_tbl->camera_flash_high(flash_ctrl, &cfg);
}
...
int asus_led_dual_torch(int current1, int current2)
{
	...
	return flash_ctrl->func_tbl->camera_flash_low(flash_ctrl, &cfg);
}
static int asus_led_torch_current_setting(int current1, int current2)
{
	uint32_t curr = 0, max_current = 0;
	int32_t i = 0;
	int torch_current[2]={current1,current2};
	for (i = 0; i < 2; i++)
	{
		if (flash_ctrl->torch_trigger[i])
		{
			max_current = flash_ctrl->torch_max_current[i];
			if (torch_current[i] >= 0 && torch_current[i] <= max_current)
			{
				curr = torch_current[i];
			}
			else
			{
				curr = flash_ctrl->torch_op_current[i];
...
static int asus_led_flash_current_setting(int current1, int current2)
{
	uint32_t curr = 0, max_current = 0;
	int32_t i = 0;
	int flash_current[2]={current1,current2};
	for (i = 0; i < 2; i++)
	{
		if (flash_ctrl->flash_trigger[i])
		{
...
static int asus_led1_torch(void)
{
	return asus_led_dual_torch(200,0);
}

static int asus_led2_torch(void)
{
	return asus_led_dual_torch(0,200);
}

static int asus_led1_flash(void)
{
	return asus_led_dual_flash(800,0);
}

static int asus_led2_flash(void)
{
	return asus_led_dual_flash(0,800);
}


static int asus_led_flash_duration_setting(int duration1, int duration2)
{
	return 0;//can not implement
}
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	if(flash_ctrl->flash_driver_type != FLASH_DRIVER_PMIC)
	{
		CDBG("FLASH, flash type force set to %d -> FLASH_DRIVER_PMIC",FLASH_DRIVER_PMIC);
		flash_ctrl->flash_driver_type = FLASH_DRIVER_PMIC;
	}
	init_asus_led(&flash_control_table);
	CDBG("probe success\n");
	return rc;
}
...
static asus_flash_function_table_t flash_control_table =
{
	.asus_led_init = asus_led_init,
	.asus_led_off = asus_led_off,
	.asus_led_release = asus_led_release,
	.asus_led_torch = asus_led_torch,
	.asus_led_flash = asus_led_flash,
	.asus_led1_torch = asus_led1_torch,
	.asus_led1_flash = asus_led1_flash,
	.asus_led2_torch = asus_led2_torch,
	.asus_led2_flash = asus_led2_flash,
	.asus_led_torch_current_setting = asus_led_torch_current_setting,
	.asus_led_flash_current_setting = asus_led_flash_current_setting,
	.asus_led_flash_duration_setting = asus_led_flash_duration_setting,
	.asus_led_dual_torch = asus_led_dual_torch,
	.asus_led_dual_flash = asus_led_dual_flash
};
