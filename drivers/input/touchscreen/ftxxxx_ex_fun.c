...
/*ASUS_BSP ander : setting priority +++ */
#ifdef ASUS_FACTORY_BUILD
#define Focal_RW_ATTR (S_IRUGO | S_IWUSR | S_IWGRP)
...
//	printk(KERN_WARNING"Firmware version = %d.%d.%d\n",
//		data->fw_ver[0], data->fw_ver[1], data->fw_ver[2]);
}
/***********************************************/
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	int i_ret;
	int fw_len;
	printk("[Focal][Touch]fts_ctpm_fw_upgrade_with_i_file\n");
/*
	switch(ftxxxx_ts->lcd_vendor)
	  {
		case '0' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==CTPM_FW_ZE550KL_GIS_TM\n");
			CTPM_FW = CTPM_FW_ZE550KL_GIS_TM;
			CTPM_FW_length = sizeof(CTPM_FW_ZE550KL_GIS_TM);
			break;
		case '1' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==0x30\n");
			CTPM_FW = CTPM_FW_ZE550KL_GIS_TM;
			CTPM_FW_length = sizeof(CTPM_FW_ZE550KL_GIS_TM);
			break;
		case '2' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==CTPM_FW_ZE550KL_TPK_TM\n");
			CTPM_FW = CTPM_FW_ZE550KL_TPK_TM;
			CTPM_FW_length = sizeof(CTPM_FW_ZE550KL_TPK_TM);
			break;
		case '3' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==CTPM_FW_ZE551KL_GIS_TM\n");
		...
		case '4':
			...
			break;
		case '5' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==CTPM_FW_ZE551KL_TPK_AUO\n");
			CTPM_FW = CTPM_FW_ZE551KL_TPK_AUO;
			CTPM_FW_length = sizeof(CTPM_FW_ZE551KL_TPK_AUO);
			break;
		case '6' :
			printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==CTPM_FW_ZE551KL_TPK_TM\n");
			CTPM_FW = CTPM_FW_ZE551KL_TPK_TM;
			CTPM_FW_length = sizeof(CTPM_FW_ZE551KL_TPK_TM);
			break;
		}
*/
	printk("[Focal][TOUCH_ERR]_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==TP_5446_FW\n");
	CTPM_FW = TP_5446_FW;
	CTPM_FW_length = sizeof(TP_5446_FW);	
...
*/
	printk("[Focal][Touch]fts_ctpm_auto_upgrade ftxxxx_ts->lcd_vendor==TP_5446_FW\n");
	CTPM_FW = TP_5446_FW;
	CTPM_FW_length = sizeof(TP_5446_FW);		
	
	//printk(KERN_WARNING"here will judge wether to update the firmware\n");
...
static ssize_t ftxxxx_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	u8 fwver = 0;
/*	struct i2c_client *client = container_of(dev, struct i2c_client, dev); ASUS ander use globle ftxxxx_ts data*/
...
/* +++ASUS ander add for get tp raw data +++*/
