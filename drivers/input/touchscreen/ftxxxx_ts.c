...
/*ASUS_BSP ander kung: add for debug mask +++ */
#include <linux/module.h>
/* Debug levels */
#define NO_DEBUG       0
...
/* +++ asus ander add for print touch location +++ */
...
/* ander add for i2c retey and if i2c error countor > 10 reset IC */
...
//ASUS_BSP Aric_Su+++ [Scorpio][Touch][NA][Spec] add for FHD panel
...
/*ASUS_BSP ander : add for creating virtual_key_maps +++*/
#define VIRTURAL_KEY
#ifdef VIRTURAL_KEY
...
static ssize_t focalTP_virtual_keys_register(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	char *virtual_keys ;
		/*if(ftxxxx_ts->lcd_vendor < '3')
	  	{
	  	 printk("[Focal][Touch]is hd virtualkey");
	  			virtual_keys = 	__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":140:1341:180:100" "\n" \
	  	...
	  	{*/
		printk("[Focal][Touch]is fhd   virtual key");
...
{
/*	struct ftxxxx_ts_data *ftxxxx_ts = dev_id; ASUS ander use globle ftxxxx_ts data*/
	int ret = 0;
#ifdef FTS_GESTRUE/*zax 20140922*/
	u8 state;
#endif
...
//	printk(KERN_EMERG "[Focal][Touch] %s : Touch suspend +++ ++\n", __func__);
	...
	if ((ftxxxx_ts->dclick_mode_eable == true) || (ftxxxx_ts->gesture_mode_eable == true)) {
		...
		ftxxxx_write_reg(ftxxxx_ts->client, buf[0], buf[1]);
		if (ts->ts_pinctrl) {
			int err = pinctrl_select_state(ts->ts_pinctrl,
						ts->pinctrl_state_suspend);
			if (err < 0)
				printk("[Focal][Touch]Cannot get suspend pinctrl state\n");
		}
	}
#else
...
static void focal_resume_work(struct work_struct *work)
{
	...
	if ((ftxxxx_ts->dclick_mode_eable == true) ||(ftxxxx_ts->gesture_mode_eable == true)) {
		...
		if (ftxxxx_ts->ts_pinctrl) {
			int err = pinctrl_select_state(ftxxxx_ts->ts_pinctrl,
					ftxxxx_ts->pinctrl_state_active);
			if (err < 0)
				printk("[Focal][Touch]Cannot get active pinctrl state\n");
		}
...
static int fts_pinctrl_init(struct ftxxxx_ts_data *ftxxxx_ts)
{
	int retval;

	/* Get pinctrl if target uses pinctrl */
	ftxxxx_ts->ts_pinctrl = devm_pinctrl_get(&(ftxxxx_ts->client->dev));
	if (IS_ERR_OR_NULL(ftxxxx_ts->ts_pinctrl)) {
		retval = PTR_ERR(ftxxxx_ts->ts_pinctrl);
		dev_dbg(&ftxxxx_ts->client->dev,
			"Target does not use pinctrl %d\n", retval);
		goto err_pinctrl_get;
	}

	ftxxxx_ts->pinctrl_state_active
		= pinctrl_lookup_state(ftxxxx_ts->ts_pinctrl,
				PINCTRL_STATE_ACTIVE);
	if (IS_ERR_OR_NULL(ftxxxx_ts->pinctrl_state_active)) {
		retval = PTR_ERR(ftxxxx_ts->pinctrl_state_active);
		dev_err(&ftxxxx_ts->client->dev,
			"Can not lookup %s pinstate %d\n",
			PINCTRL_STATE_ACTIVE, retval);
		goto err_pinctrl_lookup;
	}

	ftxxxx_ts->pinctrl_state_suspend
		= pinctrl_lookup_state(ftxxxx_ts->ts_pinctrl,
			PINCTRL_STATE_SUSPEND);
	if (IS_ERR_OR_NULL(ftxxxx_ts->pinctrl_state_suspend)) {
		retval = PTR_ERR(ftxxxx_ts->pinctrl_state_suspend);
		dev_err(&ftxxxx_ts->client->dev,
			"Can not lookup %s pinstate %d\n",
	...
	ftxxxx_ts->pinctrl_state_release
		= pinctrl_lookup_state(ftxxxx_ts->ts_pinctrl,
			PINCTRL_STATE_RELEASE);
	if (IS_ERR_OR_NULL(ftxxxx_ts->pinctrl_state_release)) {
		retval = PTR_ERR(ftxxxx_ts->pinctrl_state_release);
		dev_dbg(&ftxxxx_ts->client->dev,
			"Can not lookup %s pinstate %d\n",
...
err_pinctrl_lookup:
	devm_pinctrl_put(ftxxxx_ts->ts_pinctrl);
err_pinctrl_get:
	ftxxxx_ts->ts_pinctrl = NULL;
	return retval;
}
...
/* +++asus ander add for pars dt info+++ */
...
static int ftxxxx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//extern char asus_lcd_id[2];
	//touch_control=1
	printk("[Focal][Touch] FTxxxx probe process Start !GTP I2C Address: 0x%02x\n", client->addr);
...
//	printk("[Focal][Touch]ftxxxx_ts->lcd_vendor=0x%x",ftxxxx_ts->lcd_vendor);
	/*if(ftxxxx_ts->lcd_vendor < 0x33)
	{
		printk("[Focal][Touch]is hd");
		ftxxxx_ts->x_max=720;
		ftxxxx_ts->y_max=1280;
	}
	else
	{
		printk("[Focal][Touch]is fhd");*/
		ftxxxx_ts->x_max=1080;
		ftxxxx_ts->y_max=1920;
	/*}
	switch (ftxxxx_ts->lcd_vendor)
	{
	...
	//	printk("[Focal][Touch]ftxxxx_ts->lcd_vendor=0x%x\n",ftxxxx_ts->lcd_vendor);
	/*add by jinpeng_He for get the id pin end*/

	err = fts_pinctrl_init(ftxxxx_ts);
	if (!err && ftxxxx_ts->ts_pinctrl) {
		/*
		 * Pinctrl handle is optional. If pinctrl handle is found
		 * let pins to be configured in active state. If not
		 * found continue further without error.
		 */
		err = pinctrl_select_state(ftxxxx_ts->ts_pinctrl,
					ftxxxx_ts->pinctrl_state_active);
		if (err < 0) {
			printk("[Focal][Touch]failed to select pin to active state");
		}
	}
...
//	printk("[Focal][Touch] TP vendor ID = %d \n", ftxxxx_read_tp_id());
	...
	if (ftxxxx_ts->ts_pinctrl) {
		if (IS_ERR_OR_NULL(ftxxxx_ts->pinctrl_state_release)) {
			...
			err = pinctrl_select_state(ftxxxx_ts->ts_pinctrl,
					ftxxxx_ts->pinctrl_state_release);
			if (err)
				pr_err("[Focal][Touch]failed to select relase pinctrl state\n");
		}
	}
...
static int ftxxxx_ts_remove(struct i2c_client *client)
{
	...
	if (ftxxxx_ts->ts_pinctrl) {
		if (IS_ERR_OR_NULL(ftxxxx_ts->pinctrl_state_release)) {
			...
			int retval = pinctrl_select_state(ftxxxx_ts->ts_pinctrl,
					ftxxxx_ts->pinctrl_state_release);
			if (retval < 0)
				pr_err("[Focal][Touch]failed to select release pinctrl state\n");
		}
	}
