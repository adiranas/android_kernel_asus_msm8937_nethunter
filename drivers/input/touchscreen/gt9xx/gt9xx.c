...

{
    ...
    msg[0] = (addr >> 8) & 0xff;
    msg[1] = addr & 0xff;
	GTP_ERROR("[Touch]gup_get_ic_msg_fortest");
   ...
   //GTP_DEBUG_FUNC();
	...
	{     
		   ...
		   )
		{
			...
			{
				GTP_INFO("[Touch]irq run to gtp_enter_doze ");
				// clear 0x814B
				doze_status = DOZE_WAKEUP;
				doze_buf[2] = 0x00;
				gtp_i2c_write(i2c_connect_client, doze_buf, 3);
		  ...
		  (doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA) )
		  {
			  if ( ts->swipeup_mode_eable ==1)
			{
				...
				GTP_INFO("%s swipe up to light up the screen!", direction[type]);
				doze_status = DOZE_WAKEUP;
...
void goodix_usb_detection(bool plugin)
{
	if (ts == NULL || !ts->is_ready) {
		printk("[TOUCH_ERR] %s : ftxxxx_ts is null, skip \n", __func__);
		return;
	}

	if (plugin)
		ts->usb_status = 1; /*AC plug in*/
	else
		ts->usb_status = 0;	/*no AC */

	queue_work(ts->usb_wq, &ts->usb_detect_work);
}

EXPORT_SYMBOL(goodix_usb_detection);

static void goodix_cable_status(struct work_struct *work)
{
	u8 charge_in_buf[3] = {0x80, 0x40,0x06};
	u8 charge_out_buf[3] = {0x80, 0x40,0x07};
	int status = ts->usb_status;

	wake_lock(&ts->wake_lock);
	mutex_lock(&ts->g_device_mutex);
	printk("[Touch] cable_status=%d.\n", status);

	if (status == 0) {	/*no AC */
		gtp_i2c_write_no_rst(i2c_connect_client, charge_out_buf, 3);
	} else if (status == 1) {	/*AC plug in*/
		gtp_i2c_write_no_rst(i2c_connect_client, charge_in_buf, 3);
	}
...
static ssize_t switch_gesture_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
		...
		if (gesture_buf[0] == cmpchar) {
			ts->gesture_mode_eable = true;
			printk("[Touch] gesture_mode enable ! \n");
		} else
			ts->gesture_mode_eable = false;
...
static ssize_t switch_glove_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	   ...
	   if (tmp == 0) {
		 ...
		 if (ret < 0)
           {
                  GTP_ERROR("golve mode off -GTP init panel failed.");
		 	}
	    } else if (tmp == 1) {
				 ...
				 if (ret < 0)
                 {
                   GTP_ERROR("golve mode on-GTP init panel failed.");
				 }
...
static ssize_t show_tp_name(struct switch_dev *sdev, char *buf)
{
	...
	return scnprintf (buf, PAGE_SIZE,  "%s_%d_%s_%s\n", s,tp_cfg_version,tp_vendor_version,lcd_vendor_version) ;
}
/*
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
        GTP_ERROR("[Touch]I2C communication ERROR!");
    }
  ...
  if (ts->use_irq==0){
	...
	mutex_init(&ts->g_device_mutex);
	wake_lock_init(&ts->wake_lock, WAKE_LOCK_SUSPEND, "goodix_touch_wake_lock");
	//add usb status++++
	ts->usb_wq = create_singlethread_workqueue("goodix_usb_wq");
	if (!ts->usb_wq) {
		printk("[TOUCH_ERR] %s: create usb workqueue failed\n", __func__);
		return -ENOMEM;
	}
	INIT_WORK(&ts->usb_detect_work, goodix_cable_status);
	//add usb status -----
...
void goodix_ts_suspend(void)
{
    ...
    int i;
    GTP_DEBUG_FUNC();
    if ((ts == NULL) || ts->enter_update || !ts->is_ready) {
    	return;
    }
