...
int g_gpio_asus_debug; /* ASUS_BSP Freeman +++ */
...
static int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	//Set the quat status for checking the i2s bit clock is sending or not;asus_bsp++
	quat_mi2s_status = 1;
	pr_err("%s: quat_mi2s_status = 1\n", __func__);
	//Set the quat status for checking the i2s bit clock is sending or not;asus_bsp--
...
err:
	...
	if (ret < 0)
		pr_err("failed to disable sclk\n");
	//Set the quat status for checking the i2s bit clock is sending or not;asus_bsp++
	pr_err("%s: quat i2s start error, quat_mi2s_status = 0\n", __func__);
	quat_mi2s_status = 0;
	//Set the quat status for checking the i2s bit clock is sending or not;asus_bsp--
	return ret;
}
...
static void msm_quat_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	else { //Set the quat status for checking the i2s bit clock is sending or not;asus_bsp++
		    pr_err("%s: quat_mi2s_status = 0\n", __func__);
			quat_mi2s_status = 0;
		}
		//Set the quat status for checking the i2s bit clock is sending or not;asus_bsp--
}
...
/*modify for ti amp;asus_bsp++*/
#ifdef MSM8937_ZC600KL
...
static struct snd_soc_dai_link tas_stereo_dai_link[] = {
{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
		...
		//.codec_dai_name = "snd-soc-dummy-dai",
		//.codec_name = "snd-soc-dummy",
		.codec_dai_name = "tas2555 Stereo ASI1",
        .codec_name = "tas2555s.6-004d",
		.no_pcm = 1,
		.dpcm_playback = 1,
		.be_id = MSM_BACKEND_DAI_QUATERNARY_MI2S_RX,
		.be_hw_params_fixup = msm_be_hw_params_fixup,
...
#if 0
static struct snd_soc_dai_link tas_mono_dai_link[] = {
	{
		.name = LPASS_BE_QUIN_MI2S_RX,
		.stream_name = "Quinary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.5",
...
#ifdef MSM8937_ZC600KL
ARRAY_SIZE(msm8952_quat_dai_link) + //modify for ti amp;asus_bsp++;
#endif
...
codec_dai:
		...
		if (dai_link[i].codec_name && !dai_link[i].codec_of_node) {
			...
			if(strcmp(dai_link[i].codec_name,"tas2555s.6-004d")){
			pr_err("%s: codec dai name %s will be clear\n",
					__func__, dai_link[i].codec_name);
						...
						struct device *dev)
{
	...
	int len1;
    dev_err(dev, "%s(): getting into,g_ASUS_PRJ_ID=%d\n",
				__func__,g_ASUS_PRJ_ID);
	...
	if(PRJ_CAPRICORN_10W == g_ASUS_PRJ_ID || PRJ_CAPRICORN_5W == g_ASUS_PRJ_ID){	
		dev_err(dev, "%s(): No hdmi dba present, add tas dai\n",
				__func__); 
		/*memcpy(dailink + len1, tas_mono_dai_link,
				sizeof(tas_mono_dai_link));
		len1 += ARRAY_SIZE(tas_mono_dai_link);*/
		#ifdef MSM8937_ZC600KL
		memcpy(dailink + len1, tas_stereo_dai_link,
				sizeof(tas_stereo_dai_link));
		len1 += ARRAY_SIZE(tas_stereo_dai_link);
		#endif
	}else if (of_property_read_bool(dev->of_node,
				"qcom,hdmi-dba-codec-rx")) {
		dev_err(dev, "%s(): hdmi audio support present\n",
				__func__);
	...
	if (of_property_read_bool(dev->of_node,
				"qcom,split-a2dp")) {
		dev_err(dev, "%s(): split a2dp support present\n",
				__func__);
...
parse_mclk_freq:
	...
	/* ASUS_BSP Freeman +++ */
	g_gpio_asus_debug = of_get_named_gpio(pdev->dev.of_node, "ASUS_DEBUG", 0);
	if (g_gpio_asus_debug < 0)
		printk("%s: property ASUS_DEBUG not found\n", __func__);
	if(g_user_dbg_mode == 1)
		gpio_direction_output(g_gpio_asus_debug, 0);
	else
		gpio_direction_output(g_gpio_asus_debug, 1);
	/* ASUS_BSP Freeman --- */
	...
	//Create the quat checking status proc file;asus_bsp++
	create_quat_status_proc_file();
	pr_debug("%s:  after create_quat_status_proc_file\n",
				__func__);
	//Create the quat checking status proc file;asus_bsp--
...
err1:
	...
	//remove the quat checking status proc file;asus_bsp++
