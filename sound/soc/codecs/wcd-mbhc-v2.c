...
#include <linux/debugfs.h> //ASUS_BSP +++ tyree_liu add mbhc node for BMMI
...
//ASUS_BSP +++ tyree_liu add mbhc node for BMMI
ssize_t wcd_mbhc_debug_read(struct file *file, char __user *buff, size_t len, loff_t *off)
{
...
{
	mbhc->debugfs_mbhc = debugfs_create_file("wcd9xxx_mbhc", S_IFREG | S_IRUGO, NULL, mbhc, &mbhc_debug_ops);
}
