	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (strnstr(w->name, internal1_text, strlen(w->name))) {
			...
			//mei_huang+++ change register 143 bit 6 to 0,avoid micbias Short circuit,this suggestion from qcom
	...
	case SND_SOC_DAPM_POST_PMD:
		if (strnstr(w->name, internal1_text, strlen(w->name))) {
			...
			//mei_huang+++ change register 143 bit 6 to 0
...
static int msm8x16_wcd_codec_probe(struct snd_soc_codec *codec)
{
	...
	codec_status=1;//ASUS_BSP +++ tyree_liu@asus.com  add for codec_status
	/* ASUS_BSP mei_huang --- */
